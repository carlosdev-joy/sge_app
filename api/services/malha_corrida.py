...
def corrida_aberta(cur, malha: str, sentinela_erro=None):
...
def sql_corridas_das_linhas(quantos: int) -> str:
    ...
    tabela do schema."""
    dentro = (", ".join("?" for _ in range(int(quantos)))
    ...
    return ("SELECT " + _COLS + " FROM dbo.etl_malha_execucao "
            ...
            "WHERE e.data_referencia = ? AND e.malha_execucao_id IS NOT NULL "
            "AND " + filtro + ") ORDER BY id")


def corridas_das_linhas(cur, pipelines, data_ref) -> list:
...
SQL_REABRIR = (
    ...
    "+ CASE WHEN ? IS NULL THEN '' ELSE ': ' + ? END, 500), "
...
def reabrir_corrida(cur, corrida_id, reaberta_por: str, motivo=None) -> bool:
    ...
    try:
        ...
        raise
    if reabriu:
        descartar_desfecho(cur, corrida_id)
    return reabriu
...
SQL_DESCARTAR_DESFECHO = (
    ...
    "WHERE malha_execucao_id = ? "
    "AND tipo IN (" + ", ".join("?" for _ in EVENTOS_DO_DESFECHO) + ") "
...
def descartar_desfecho(cur, corrida_id) -> int:
...
def corrida_da_data(cur, malha: str, data_ref, sentinela_erro=None):
