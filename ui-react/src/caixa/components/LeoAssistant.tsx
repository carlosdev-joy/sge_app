...
const defaultQuestions = [
  "Quais são as principais pendências documentais?",
  "Analise as propostas fora da conformidade",
  "Qual o valor total em pendências?",
  "Gere um relatório das pendências críticas",
];

const LeoAssistantInner = ({ suggestedQuestions = defaultQuestions }: LeoAssistantProps) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: "Olá! Sou o Léo, seu assistente virtual. Posso te ajudar a entender os dados da página, gerar análises personalizadas e até exportar relatórios em PDF. Como posso ajudar?",
      sender: "assistant",
      timestamp: new Date(),
    },
  ]);
  ...
  const loadConversationHistory = async () => {
    try {
      const data = await apiFetch<{ conversas: ConversaHistorico[] }>(`${CHAT_ENDPOINT("leo")}/historico`);
  ...
  const getPageContext = () => {
    return {
      page: 'Gestão Comercial - Previdência',
      availableData: [
        'Status de propostas (Assinatura, Pagamento, Documentação)',
        'Pendências documentais com sub-status',
        'Propostas fora da conformidade > R$ 500k',
        'Motivos de pendências',
        'Evolução temporal de captação',
      ],
      currentMetrics: {
        pendingDocumentation: 43,
        nonCompliantOver500k: 12,
        totalValue: 'R$ 1.847.000,00',
      },
    };
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const data = await apiFetch<{ response: string }>(CHAT_ENDPOINT("leo"), {
        method: "POST",
      ...
      console.error('Error calling Leo:', error);
  ...
  const exportToPDF = () => {
    try {
      ...
      doc.text("Conversa com Léo - Assistente de Previdência", margin, yPosition);
      yPosition += 10;
      ...
      messages.forEach((message) => {
        ...
        doc.text(message.sender === "user" ? "Você:" : "Léo:", margin, yPosition);
        yPosition += 6;
      ...
      doc.save(`leo-conversa-${new Date().getTime()}.pdf`);
        ...
        <Card className="fixed bottom-6 right-6 w-96 h-[600px] shadow-2xl flex flex-col z-50 bg-card animate-scale-in">
          <div className="flex items-center justify-between p-4 border-b bg-primary text-primary-foreground rounded-t-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold">Léo</h3>
              ...
              {isLoading && (
                <div className="flex justify-start animate-fade-in">
                  ...
                  <div className="bg-muted rounded-lg p-3 flex items-center gap-3">
                    <TypingIndicator />
                    <span className="text-sm text-muted-foreground">Léo está digitando...</span>
                  </div>
                </div>
              )}
...
const LeoAssistant = (props: LeoAssistantProps) => {
  const ativos = useAssistentesIA();
  return ativos ? <LeoAssistantInner {...props} /> : null;
};
