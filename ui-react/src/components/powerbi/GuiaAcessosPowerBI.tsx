...
// ── Power BI — Como liberar acessos ──────────────────────────────
// Veio da aba Admin › Sistema › Power BI — Acessos (F4 de
// docs/spec-admin-reestruturacao.md §3.3): é texto de referência para o pedido
// ao time de Infra/Segurança, então mora na tela do Power BI, numa seção
// recolhível FECHADA por padrão no fim da página. O conteúdo é o mesmo da aba.
export const ANCORA_PBI_ACESSOS = 'como-liberar-acessos'
interface PbiLayer {
  num: string
  titulo: string
...
/**
 * Seção recolhível (padrão <details> do app — o mesmo "Gateway e limites" de
 * Admin › Agentes): fecha por padrão, abre/fecha por Enter/Espaço no resumo.
 * Sem moldura em volta do conteúdo aberto: os cartões das camadas já são
 * painéis, e painel dentro de painel pesaria.
 */
export function SecaoComoLiberarAcessos() {
  return (
    <details id={ANCORA_PBI_ACESSOS} className="group scroll-mt-4" data-secao-pbi-acessos>
      <summary className="flex items-center gap-2 px-4 py-3 bg-panel border border-edge rounded-lg cursor-pointer select-none
                          list-none [&::-webkit-details-marker]:hidden hover:bg-canvas
                          focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1A5FA8]">
        <ChevronRight className="w-4 h-4 shrink-0 text-dim transition-transform duration-150 group-open:rotate-90
                                 motion-reduce:transition-none" aria-hidden="true" />
        <h2 className="text-sm font-semibold text-ink shrink-0">Como liberar acessos</h2>
        <span className="min-w-0 truncate text-xs text-dim">
          camadas de acesso à API do Power BI e o passo a passo do pedido
        </span>
      </summary>
      {/* Em 390 px a tela rolava de lado: caminhos longos sem espaço
          (workspaces/datasets/…, endpoints) e o selo do responsável de cada
          passo (Badge é nowrap) passavam da largura. Quebra só aqui dentro. */}
      <div className="mt-4 min-w-0 [overflow-wrap:anywhere] [&_ol_.whitespace-nowrap]:whitespace-normal">
        <GuiaAcessosPowerBI />
      </div>
    </details>
  )
