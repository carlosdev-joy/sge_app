...
// (/governanca), acessível a quem não constrói malhas. Os dados continuam
// vindo de GET /malha. A visão "Grafo dep." (DependencyGraph SVG legado, cores
// dark hardcoded) MORREU na realocação — dependências agora se visualizam na
// malha (MalhaEditor).
...
function exportCsv(data: ApiPipeline[]) {
  ...
  a.download = `catalogo_pipelines_${new Date().toISOString().slice(0, 10)}.csv`
  a.click()
}
...
export function CatalogoPipelines() {
    ...
    <div className="flex flex-col gap-4">
      ...
      {bannerOpen && (
        <div className="flex items-start gap-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg px-4 py-3">
          ...
          <div className="flex-1 text-[12px] leading-relaxed text-blue-800 dark:text-blue-200">
            <strong>Catálogo de pipelines:</strong> visualize os pipelines agrupados por projeto, com seus fluxos de jobs (serial e paralelo),
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <div className="flex items-center gap-1 ml-auto">
          {/* O "Grafo dep." legado morreu na F9 — o lugar de VER dependência é
              o diagrama da malha, que respeita os dois temas. */}
          <span className="mr-1 text-[10px] text-dim">
            dependências agora se visualizam na malha
          </span>
          <button
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-800 px-4 py-3 text-sm text-red-700 dark:text-red-300">
          Erro ao carregar o catálogo: {(error as Error)?.message ?? 'Erro desconhecido'}
        </div>
      ) : filtered.length === 0 ? (
