...
run_sql_file() {
    ...
    docker exec "$CONTAINER" bash -c "
        sed 's/USE \[DMDB41\]/USE [orquestra_dev]/gi; s/\[DMDB41\]\.\[dbo\]/[orquestra_dev].[dbo]/gi' /tmp/_current.sql > /tmp/_ready.sql
    "
    docker exec "$CONTAINER" bash -c "$SQLCMD_BIN -C -S localhost -U sa -P '$SA_PASS' -d orquestra_dev -i /tmp/_ready.sql -b 2>&1 | grep -v '^$' | grep -v '^--'" || true
}
