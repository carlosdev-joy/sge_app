...
# Pipeline: DATASTAGE_MALHA_VIDA | Projeto: BI_VIDA | Dominio: GERAL
# =========================
DAG_ID        = "DATASTAGE_MALHA_VIDA"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_VIDA"
DOMAIN        = "GERAL"
PIPELINE_NAME = "DATASTAGE_MALHA_VIDA"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_MALHA_VIDA"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 3, "retry_delay": timedelta(seconds=1500)}
JOBS          = ['SeqExecCargaVida']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_MALHA_VIDA - BI_VIDA / GERAL",
    start_date=pendulum.datetime(2026, 6, 19, tz=LOCAL_TZ),
    schedule="0 2 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_VIDA', 'GERAL', 'MALHA_VIDA', 'DIARIO'],
) as dag:
    ...
    t_start_SeqExecCargaVida = PythonOperator(
        task_id="log_start_SeqExecCargaVida",
        python_callable=log_start,
        op_kwargs={"job_name": "SeqExecCargaVida", "task_key": "SeqExecCargaVida"},
    )

    t_job_SeqExecCargaVida = DataStageOperator(
        task_id="SeqExecCargaVida",
    ...
    t_end_SeqExecCargaVida = PythonOperator(
        task_id="log_end_SeqExecCargaVida",
        python_callable=log_end,
        op_kwargs={"job_name": "SeqExecCargaVida", "task_key": "SeqExecCargaVida", "upstream_task_id": "SeqExecCargaVida"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqExecCargaVida >> t_teams_start >> t_job_SeqExecCargaVida >> t_end_SeqExecCargaVida

    end_tasks = [t_end_SeqExecCargaVida]

    [t_end_SeqExecCargaVida] >> t_publish_dataset

    [t_end_SeqExecCargaVida] >> t_teams_end

    [t_end_SeqExecCargaVida] >> t_teams_error
