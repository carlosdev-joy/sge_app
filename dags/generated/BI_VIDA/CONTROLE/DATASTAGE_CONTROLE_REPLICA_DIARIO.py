...
# Pipeline: DATASTAGE_CONTROLE_REPLICA_DIARIO | Projeto: BI_VIDA | Dominio: CONTROLE
# =========================
DAG_ID        = "DATASTAGE_CONTROLE_REPLICA_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_VIDA"
DOMAIN        = "CONTROLE"
PIPELINE_NAME = "DATASTAGE_CONTROLE_REPLICA_DIARIO"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_CONTROLE_REPLICA_DIARIO - BI_VIDA / CONTROLE",
    start_date=pendulum.datetime(2026, 6, 15, tz=LOCAL_TZ),
    schedule="30 8 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_VIDA', 'CONTROLE', 'DATASTAGE', 'DIARIO'],
) as dag:
    ...
    t_start_ControleReplica = PythonOperator(
        task_id="log_start_ControleReplica",
        python_callable=log_start,
        op_kwargs={"job_name": "ControleReplica", "task_key": "ControleReplica"},
    )

    t_job_ControleReplica = DataStageOperator(
        task_id="ControleReplica",
        project=PROJECT_NAME,
        job_name="ControleReplica",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_ControleReplica = PythonOperator(
        task_id="log_end_ControleReplica",
        python_callable=log_end,
        op_kwargs={"job_name": "ControleReplica", "task_key": "ControleReplica", "upstream_task_id": "ControleReplica"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_ControleReplica >> t_teams_start >> t_job_ControleReplica >> t_end_ControleReplica

    end_tasks = [t_end_ControleReplica]

    [t_end_ControleReplica] >> t_publish_dataset

    [t_end_ControleReplica] >> t_teams_end

    [t_end_ControleReplica] >> t_teams_error
