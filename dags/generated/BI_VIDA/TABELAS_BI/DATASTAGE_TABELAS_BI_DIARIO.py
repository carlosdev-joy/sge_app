...
# Pipeline: DATASTAGE_TABELAS_BI_DIARIO | Projeto: BI_VIDA | Dominio: TABELAS_BI
# =========================
DAG_ID        = "DATASTAGE_TABELAS_BI_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_VIDA"
DOMAIN        = "TABELAS_BI"
PIPELINE_NAME = "DATASTAGE_TABELAS_BI_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_TABELAS_BI_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 3, "retry_delay": timedelta(seconds=300)}
JOBS          = ['StgVida_ContratoBilheteExtracao', 'StgVida_ContratoVgapExtracao', 'StgVida_ContratoVidaExtracao', 'StgVida_ContratoBilheteCarga', 'StgVida_ContratoVidaCarga', 'StgVida_ContratoVgapCarga']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_TABELAS_BI_DIARIO - BI_VIDA / TABELAS_BI",
    start_date=pendulum.datetime(2026, 6, 15, tz=LOCAL_TZ),
    schedule="7 23 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_VIDA', 'TABELAS_BI', 'DATASTAGE', 'DIARIO'],
) as dag:
    ...
    t_start_StgVida_ContratoBilheteExtracao = PythonOperator(
        task_id="log_start_StgVida_ContratoBilheteExtracao",
        python_callable=log_start,
        op_kwargs={"job_name": "StgVida_ContratoBilheteExtracao", "task_key": "StgVida_ContratoBilheteExtracao"},
    )

    t_job_StgVida_ContratoBilheteExtracao = DataStageOperator(
        task_id="StgVida_ContratoBilheteExtracao",
        project=PROJECT_NAME,
        job_name="StgVida_ContratoBilheteExtracao",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_StgVida_ContratoBilheteExtracao = PythonOperator(
        task_id="log_end_StgVida_ContratoBilheteExtracao",
        python_callable=log_end,
        op_kwargs={"job_name": "StgVida_ContratoBilheteExtracao", "task_key": "StgVida_ContratoBilheteExtracao", "upstream_task_id": "StgVida_ContratoBilheteExtracao"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_start_StgVida_ContratoVgapExtracao = PythonOperator(
        task_id="log_start_StgVida_ContratoVgapExtracao",
        python_callable=log_start,
        op_kwargs={"job_name": "StgVida_ContratoVgapExtracao", "task_key": "StgVida_ContratoVgapExtracao"},
    )

    t_job_StgVida_ContratoVgapExtracao = DataStageOperator(
        task_id="StgVida_ContratoVgapExtracao",
        project=PROJECT_NAME,
        job_name="StgVida_ContratoVgapExtracao",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_StgVida_ContratoVgapExtracao = PythonOperator(
        task_id="log_end_StgVida_ContratoVgapExtracao",
        python_callable=log_end,
        op_kwargs={"job_name": "StgVida_ContratoVgapExtracao", "task_key": "StgVida_ContratoVgapExtracao", "upstream_task_id": "StgVida_ContratoVgapExtracao"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_start_StgVida_ContratoVidaExtracao = PythonOperator(
        task_id="log_start_StgVida_ContratoVidaExtracao",
        python_callable=log_start,
        op_kwargs={"job_name": "StgVida_ContratoVidaExtracao", "task_key": "StgVida_ContratoVidaExtracao"},
    )

    t_job_StgVida_ContratoVidaExtracao = DataStageOperator(
        task_id="StgVida_ContratoVidaExtracao",
        project=PROJECT_NAME,
        job_name="StgVida_ContratoVidaExtracao",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_StgVida_ContratoVidaExtracao = PythonOperator(
        task_id="log_end_StgVida_ContratoVidaExtracao",
        python_callable=log_end,
        op_kwargs={"job_name": "StgVida_ContratoVidaExtracao", "task_key": "StgVida_ContratoVidaExtracao", "upstream_task_id": "StgVida_ContratoVidaExtracao"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_start_StgVida_ContratoBilheteCarga = PythonOperator(
        task_id="log_start_StgVida_ContratoBilheteCarga",
        python_callable=log_start,
        op_kwargs={"job_name": "StgVida_ContratoBilheteCarga", "task_key": "StgVida_ContratoBilheteCarga"},
    )

    t_job_StgVida_ContratoBilheteCarga = DataStageOperator(
        task_id="StgVida_ContratoBilheteCarga",
        project=PROJECT_NAME,
        job_name="StgVida_ContratoBilheteCarga",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_StgVida_ContratoBilheteCarga = PythonOperator(
        task_id="log_end_StgVida_ContratoBilheteCarga",
        python_callable=log_end,
        op_kwargs={"job_name": "StgVida_ContratoBilheteCarga", "task_key": "StgVida_ContratoBilheteCarga", "upstream_task_id": "StgVida_ContratoBilheteCarga"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_start_StgVida_ContratoVidaCarga = PythonOperator(
        task_id="log_start_StgVida_ContratoVidaCarga",
        python_callable=log_start,
        op_kwargs={"job_name": "StgVida_ContratoVidaCarga", "task_key": "StgVida_ContratoVidaCarga"},
    )

    t_job_StgVida_ContratoVidaCarga = DataStageOperator(
        task_id="StgVida_ContratoVidaCarga",
        project=PROJECT_NAME,
        job_name="StgVida_ContratoVidaCarga",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_StgVida_ContratoVidaCarga = PythonOperator(
        task_id="log_end_StgVida_ContratoVidaCarga",
        python_callable=log_end,
        op_kwargs={"job_name": "StgVida_ContratoVidaCarga", "task_key": "StgVida_ContratoVidaCarga", "upstream_task_id": "StgVida_ContratoVidaCarga"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_start_StgVida_ContratoVgapCarga = PythonOperator(
        task_id="log_start_StgVida_ContratoVgapCarga",
        python_callable=log_start,
        op_kwargs={"job_name": "StgVida_ContratoVgapCarga", "task_key": "StgVida_ContratoVgapCarga"},
    )

    t_job_StgVida_ContratoVgapCarga = DataStageOperator(
        task_id="StgVida_ContratoVgapCarga",
        project=PROJECT_NAME,
        job_name="StgVida_ContratoVgapCarga",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_StgVida_ContratoVgapCarga = PythonOperator(
        task_id="log_end_StgVida_ContratoVgapCarga",
        python_callable=log_end,
        op_kwargs={"job_name": "StgVida_ContratoVgapCarga", "task_key": "StgVida_ContratoVgapCarga", "upstream_task_id": "StgVida_ContratoVgapCarga"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_StgVida_ContratoBilheteExtracao >> t_teams_start >> t_job_StgVida_ContratoBilheteExtracao >> t_end_StgVida_ContratoBilheteExtracao

    t_check_agenda >> t_start_StgVida_ContratoVgapExtracao >> t_job_StgVida_ContratoVgapExtracao >> t_end_StgVida_ContratoVgapExtracao

    t_check_agenda >> t_start_StgVida_ContratoVidaExtracao >> t_job_StgVida_ContratoVidaExtracao >> t_end_StgVida_ContratoVidaExtracao

    [t_end_StgVida_ContratoBilheteExtracao, t_end_StgVida_ContratoVgapExtracao, t_end_StgVida_ContratoVidaExtracao] >> t_start_StgVida_ContratoBilheteCarga >> t_job_StgVida_ContratoBilheteCarga >> t_end_StgVida_ContratoBilheteCarga

    [t_end_StgVida_ContratoBilheteExtracao, t_end_StgVida_ContratoVgapExtracao, t_end_StgVida_ContratoVidaExtracao] >> t_start_StgVida_ContratoVidaCarga >> t_job_StgVida_ContratoVidaCarga >> t_end_StgVida_ContratoVidaCarga

    [t_end_StgVida_ContratoBilheteExtracao, t_end_StgVida_ContratoVgapExtracao, t_end_StgVida_ContratoVidaExtracao] >> t_start_StgVida_ContratoVgapCarga >> t_job_StgVida_ContratoVgapCarga >> t_end_StgVida_ContratoVgapCarga

    end_tasks = [t_end_StgVida_ContratoBilheteExtracao, t_end_StgVida_ContratoVgapExtracao, t_end_StgVida_ContratoVidaExtracao, t_end_StgVida_ContratoBilheteCarga, t_end_StgVida_ContratoVidaCarga, t_end_StgVida_ContratoVgapCarga]

    [t_end_StgVida_ContratoBilheteExtracao, t_end_StgVida_ContratoVgapExtracao, t_end_StgVida_ContratoVidaExtracao, t_end_StgVida_ContratoBilheteCarga, t_end_StgVida_ContratoVidaCarga, t_end_StgVida_ContratoVgapCarga] >> t_publish_dataset

    [t_end_StgVida_ContratoBilheteExtracao, t_end_StgVida_ContratoVgapExtracao, t_end_StgVida_ContratoVidaExtracao, t_end_StgVida_ContratoBilheteCarga, t_end_StgVida_ContratoVidaCarga, t_end_StgVida_ContratoVgapCarga] >> t_teams_end

    [t_end_StgVida_ContratoBilheteExtracao, t_end_StgVida_ContratoVgapExtracao, t_end_StgVida_ContratoVidaExtracao, t_end_StgVida_ContratoBilheteCarga, t_end_StgVida_ContratoVidaCarga, t_end_StgVida_ContratoVgapCarga] >> t_teams_error
