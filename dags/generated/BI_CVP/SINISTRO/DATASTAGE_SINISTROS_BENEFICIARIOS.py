...
# Pipeline: DATASTAGE_SINISTROS_BENEFICIARIOS | Projeto: BI_CVP | Dominio: SINISTRO
# =========================
DAG_ID        = "DATASTAGE_SINISTROS_BENEFICIARIOS"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SINISTRO"
PIPELINE_NAME = "DATASTAGE_SINISTROS_BENEFICIARIOS"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SINISTROS_BENEFICIARIOS - BI_CVP / SINISTRO",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="0 9 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'SINISTRO', 'BENEFICIARIOS'],
) as dag:
    ...
    t_start_SEQ_SINISTROS_BENEFICIARIOS = PythonOperator(
        task_id='log_start_SEQ_SINISTROS_BENEFICIARIOS',
        python_callable=log_start,
        op_kwargs={"job_name": 'SEQ_SINISTROS_BENEFICIARIOS', "task_key": 'SEQ_SINISTROS_BENEFICIARIOS'},
    )

    t_job_SEQ_SINISTROS_BENEFICIARIOS = DataStageOperator(
        task_id='SEQ_SINISTROS_BENEFICIARIOS',
        project=PROJECT_NAME,
        job_name='SEQ_SINISTROS_BENEFICIARIOS',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_SINISTROS_BENEFICIARIOS = PythonOperator(
        task_id='log_end_SEQ_SINISTROS_BENEFICIARIOS',
        python_callable=log_end,
        op_kwargs={"job_name": 'SEQ_SINISTROS_BENEFICIARIOS', "task_key": 'SEQ_SINISTROS_BENEFICIARIOS', "upstream_task_id": 'SEQ_SINISTROS_BENEFICIARIOS'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SEQ_SINISTROS_BENEFICIARIOS >> t_job_SEQ_SINISTROS_BENEFICIARIOS >> t_end_SEQ_SINISTROS_BENEFICIARIOS

    end_tasks = [t_end_SEQ_SINISTROS_BENEFICIARIOS]

    [t_end_SEQ_SINISTROS_BENEFICIARIOS] >> t_publish_dataset

    [t_end_SEQ_SINISTROS_BENEFICIARIOS] >> t_teams_error
