...
# Pipeline: TESTE2 | Projeto: BI_CVP | Dominio: PORTAL_ECONOMIARIO
# =========================
DAG_ID        = "TESTE2"
...
with DAG(
    ...
    description="Pipeline TESTE2 - BI_CVP / PORTAL_ECONOMIARIO",
    start_date=pendulum.datetime(2026, 1, 1, tz=LOCAL_TZ),
    schedule="0 6 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'PORTAL_ECONOMIARIO', 'TESTE'],
) as dag:
    ...
    t_start_Seq_Flexibiliza__o_Cobran_a = PythonOperator(
        task_id="log_start_Seq_Flexibilização_Cobrança",
        python_callable=log_start,
        op_kwargs={"job_name": "Seq_Flexibilização_Cobrança", "task_key": "Seq_Flexibilização_Cobrança"},
    )

    t_job_Seq_Flexibiliza__o_Cobran_a = DataStageOperator(
        task_id="Seq_Flexibilização_Cobrança",
        project=PROJECT_NAME,
        job_name="Seq_Flexibilização_Cobrança",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_Seq_Flexibiliza__o_Cobran_a = PythonOperator(
        task_id="log_end_Seq_Flexibilização_Cobrança",
        python_callable=log_end,
        op_kwargs={"job_name": "Seq_Flexibilização_Cobrança", "task_key": "Seq_Flexibilização_Cobrança", "upstream_task_id": "Seq_Flexibilização_Cobrança"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Seq_Flexibiliza__o_Cobran_a >> t_teams_start >> t_job_Seq_Flexibiliza__o_Cobran_a >> t_end_Seq_Flexibiliza__o_Cobran_a

    end_tasks = [t_end_Seq_Flexibiliza__o_Cobran_a]

    [t_end_Seq_Flexibiliza__o_Cobran_a] >> t_publish_dataset

    [t_end_Seq_Flexibiliza__o_Cobran_a] >> t_teams_end

    [t_end_Seq_Flexibiliza__o_Cobran_a] >> t_teams_error
