...
# Pipeline: DATASTAGE_COMISSAO_ATIVA_AUTO_MENSAL | Projeto: BI_CVP | Dominio: COMISSAO
# =========================
DAG_ID        = "DATASTAGE_COMISSAO_ATIVA_AUTO_MENSAL"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "COMISSAO"
PIPELINE_NAME = "DATASTAGE_COMISSAO_ATIVA_AUTO_MENSAL"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_COMISSAO_ATIVA_AUTO_MENSAL"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SeqBiCvp_ComissaoAtiva_auto']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_COMISSAO_ATIVA_AUTO_MENSAL - BI_CVP / COMISSAO",
    start_date=pendulum.datetime(2026, 7, 5, tz=LOCAL_TZ),
    schedule="0 12 5 * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'COMISSAO', 'DATASTAGE', 'MENSAL'],
) as dag:
    ...
    t_start_SeqBiCvp_ComissaoAtiva_auto = PythonOperator(
        task_id="log_start_SeqBiCvp_ComissaoAtiva_auto",
        python_callable=log_start,
        op_kwargs={"job_name": "SeqBiCvp_ComissaoAtiva_auto", "task_key": "SeqBiCvp_ComissaoAtiva_auto"},
    )

    t_job_SeqBiCvp_ComissaoAtiva_auto = DataStageOperator(
        task_id="SeqBiCvp_ComissaoAtiva_auto",
        project=PROJECT_NAME,
        job_name="SeqBiCvp_ComissaoAtiva_auto",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqBiCvp_ComissaoAtiva_auto = PythonOperator(
        task_id="log_end_SeqBiCvp_ComissaoAtiva_auto",
        python_callable=log_end,
        op_kwargs={"job_name": "SeqBiCvp_ComissaoAtiva_auto", "task_key": "SeqBiCvp_ComissaoAtiva_auto", "upstream_task_id": "SeqBiCvp_ComissaoAtiva_auto"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqBiCvp_ComissaoAtiva_auto >> t_job_SeqBiCvp_ComissaoAtiva_auto >> t_end_SeqBiCvp_ComissaoAtiva_auto

    end_tasks = [t_end_SeqBiCvp_ComissaoAtiva_auto]

    [t_end_SeqBiCvp_ComissaoAtiva_auto] >> t_publish_dataset

    [t_end_SeqBiCvp_ComissaoAtiva_auto] >> t_teams_error
