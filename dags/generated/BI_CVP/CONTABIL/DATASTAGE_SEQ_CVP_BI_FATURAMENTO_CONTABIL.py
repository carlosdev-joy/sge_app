...
# Pipeline: DATASTAGE_SEQ_CVP_BI_FATURAMENTO_CONTABIL | Projeto: BI_CVP | Dominio: CONTABIL
# =========================
DAG_ID        = "DATASTAGE_SEQ_CVP_BI_FATURAMENTO_CONTABIL"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "CONTABIL"
PIPELINE_NAME = "DATASTAGE_SEQ_CVP_BI_FATURAMENTO_CONTABIL"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_SEQ_CVP_BI_FATURAMENTO_CONTABIL"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SeqCvpBi_FaturamentoContabil']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SEQ_CVP_BI_FATURAMENTO_CONTABIL - BI_CVP / CONTABIL",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="15 8 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'CONTABIL'],
) as dag:
    ...
    t_start_SeqCvpBi_FaturamentoContabil = PythonOperator(
        task_id='log_start_SeqCvpBi_FaturamentoContabil',
        python_callable=log_start,
        op_kwargs={"job_name": 'SeqCvpBi_FaturamentoContabil', "task_key": 'SeqCvpBi_FaturamentoContabil'},
    )

    t_job_SeqCvpBi_FaturamentoContabil = DataStageOperator(
        task_id='SeqCvpBi_FaturamentoContabil',
        project=PROJECT_NAME,
        job_name='SeqCvpBi_FaturamentoContabil',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqCvpBi_FaturamentoContabil = PythonOperator(
        task_id='log_end_SeqCvpBi_FaturamentoContabil',
        python_callable=log_end,
        op_kwargs={"job_name": 'SeqCvpBi_FaturamentoContabil', "task_key": 'SeqCvpBi_FaturamentoContabil', "upstream_task_id": 'SeqCvpBi_FaturamentoContabil'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqCvpBi_FaturamentoContabil >> t_job_SeqCvpBi_FaturamentoContabil >> t_end_SeqCvpBi_FaturamentoContabil

    end_tasks = [t_end_SeqCvpBi_FaturamentoContabil]

    [t_end_SeqCvpBi_FaturamentoContabil] >> t_publish_dataset

    [t_end_SeqCvpBi_FaturamentoContabil] >> t_teams_error
