...
# Pipeline: GROUPS_MEMBERS_GENESYS | Projeto: BI_CVP | Dominio: GENESYS
# =========================
DAG_ID        = "GROUPS_MEMBERS_GENESYS"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "GENESYS"
PIPELINE_NAME = "GROUPS_MEMBERS_GENESYS"
...
DATASET_URI   = "orq://pipeline/GROUPS_MEMBERS_GENESYS"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 2, "retry_delay": timedelta(seconds=300)}
JOBS          = ['Sequence_BiCvp_Groups']
FLOW_JOBS     = ['Sequence_BiCvp_Groups']
...
with DAG(
    ...
    description="Pipeline GROUPS_MEMBERS_GENESYS - BI_CVP / GENESYS",
    start_date=pendulum.datetime(2026, 6, 18, tz=LOCAL_TZ),
    schedule="30 2 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'GENESYS', 'GROUPS', 'MEMBERS'],
) as dag:
    ...
    t_start_Sequence_BiCvp_Groups = PythonOperator(
        task_id='log_start_Sequence_BiCvp_Groups',
        python_callable=log_start,
        op_kwargs={"job_name": 'Sequence_BiCvp_Groups', "task_key": 'Sequence_BiCvp_Groups'},
    )

    t_job_Sequence_BiCvp_Groups = DataStageOperator(
        task_id='Sequence_BiCvp_Groups',
        project=PROJECT_NAME,
        job_name='Sequence_BiCvp_Groups',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_Sequence_BiCvp_Groups = PythonOperator(
        task_id='log_end_Sequence_BiCvp_Groups',
        python_callable=log_end,
        op_kwargs={"job_name": 'Sequence_BiCvp_Groups', "task_key": 'Sequence_BiCvp_Groups', "upstream_task_id": 'Sequence_BiCvp_Groups'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_BiCvp_Groups >> t_teams_start >> t_job_Sequence_BiCvp_Groups >> t_end_Sequence_BiCvp_Groups

    end_tasks = [t_end_Sequence_BiCvp_Groups]

    [t_end_Sequence_BiCvp_Groups] >> t_publish_dataset

    [t_end_Sequence_BiCvp_Groups] >> t_teams_end

    [t_end_Sequence_BiCvp_Groups] >> t_teams_error
