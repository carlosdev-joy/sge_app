...
# Pipeline: QUEUES_GENESYS_DIARIO | Projeto: BI_CVP | Dominio: GENESYS
# =========================
DAG_ID        = "QUEUES_GENESYS_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "GENESYS"
PIPELINE_NAME = "QUEUES_GENESYS_DIARIO"
...
with DAG(
    ...
    description="Pipeline QUEUES_GENESYS_DIARIO - BI_CVP / GENESYS",
    start_date=pendulum.datetime(2026, 6, 18, tz=LOCAL_TZ),
    schedule="30 4 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'GENESYS', 'QUEUES', 'CVP'],
) as dag:
    ...
    t_start_Sequence_BiCvp_Queues = PythonOperator(
        task_id="log_start_Sequence_BiCvp_Queues",
        python_callable=log_start,
        op_kwargs={"job_name": "Sequence_BiCvp_Queues", "task_key": "Sequence_BiCvp_Queues"},
    )

    t_job_Sequence_BiCvp_Queues = DataStageOperator(
        task_id="Sequence_BiCvp_Queues",
        project=PROJECT_NAME,
        job_name="Sequence_BiCvp_Queues",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
        verbose_log=True,
    )

    t_end_Sequence_BiCvp_Queues = PythonOperator(
        task_id="log_end_Sequence_BiCvp_Queues",
        python_callable=log_end,
        op_kwargs={"job_name": "Sequence_BiCvp_Queues", "task_key": "Sequence_BiCvp_Queues", "upstream_task_id": "Sequence_BiCvp_Queues"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_BiCvp_Queues >> t_teams_start >> t_job_Sequence_BiCvp_Queues >> t_end_Sequence_BiCvp_Queues

    end_tasks = [t_end_Sequence_BiCvp_Queues]

    [t_end_Sequence_BiCvp_Queues] >> t_publish_dataset

    [t_end_Sequence_BiCvp_Queues] >> t_teams_end

    [t_end_Sequence_BiCvp_Queues] >> t_teams_error
