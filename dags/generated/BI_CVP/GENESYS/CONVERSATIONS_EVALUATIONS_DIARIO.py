...
# Pipeline: CONVERSATIONS_EVALUATIONS_DIARIO | Projeto: BI_CVP | Dominio: GENESYS
# =========================
DAG_ID        = "CONVERSATIONS_EVALUATIONS_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "GENESYS"
PIPELINE_NAME = "CONVERSATIONS_EVALUATIONS_DIARIO"
...
DATASET_URI   = "orq://pipeline/CONVERSATIONS_EVALUATIONS_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 3, "retry_delay": timedelta(seconds=300)}
JOBS          = ['Sequence_BiCvp_ConversationsEvaluations']
...
with DAG(
    ...
    description="Pipeline CONVERSATIONS_EVALUATIONS_DIARIO - BI_CVP / GENESYS",
    start_date=pendulum.datetime(2026, 1, 1, tz=LOCAL_TZ),
    schedule="0 2 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'GENESYS', 'CONVERSATIONS', 'EVALUATIONS', 'CVP'],
) as dag:
    ...
    t_start_Sequence_BiCvp_ConversationsEvaluations = PythonOperator(
        task_id="log_start_Sequence_BiCvp_ConversationsEvaluations",
        python_callable=log_start,
        op_kwargs={"job_name": "Sequence_BiCvp_ConversationsEvaluations", "task_key": "Sequence_BiCvp_ConversationsEvaluations"},
    )

    t_job_Sequence_BiCvp_ConversationsEvaluations = DataStageOperator(
        task_id="Sequence_BiCvp_ConversationsEvaluations",
        project=PROJECT_NAME,
        job_name="Sequence_BiCvp_ConversationsEvaluations",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_Sequence_BiCvp_ConversationsEvaluations = PythonOperator(
        task_id="log_end_Sequence_BiCvp_ConversationsEvaluations",
        python_callable=log_end,
        op_kwargs={"job_name": "Sequence_BiCvp_ConversationsEvaluations", "task_key": "Sequence_BiCvp_ConversationsEvaluations", "upstream_task_id": "Sequence_BiCvp_ConversationsEvaluations"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_BiCvp_ConversationsEvaluations >> t_teams_start >> t_job_Sequence_BiCvp_ConversationsEvaluations >> t_end_Sequence_BiCvp_ConversationsEvaluations

    end_tasks = [t_end_Sequence_BiCvp_ConversationsEvaluations]

    [t_end_Sequence_BiCvp_ConversationsEvaluations] >> t_publish_dataset

    [t_end_Sequence_BiCvp_ConversationsEvaluations] >> t_teams_end

    [t_end_Sequence_BiCvp_ConversationsEvaluations] >> t_teams_error
