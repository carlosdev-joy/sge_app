...
with DAG(
    ...
    description="Pipeline USERS_GENESYS_DIARIO - BI_CVP / GENESYS",
    start_date=pendulum.datetime(2026, 6, 18, tz=LOCAL_TZ),
    schedule="0 4 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'GENESYS', 'USERS', 'CVP'],
) as dag:
    ...
    t_start_Sequence_BiCvp_Users = PythonOperator(
        task_id="log_start_Sequence_BiCvp_Users",
        python_callable=log_start,
        op_kwargs={"job_name": "Sequence_BiCvp_Users", "task_key": "Sequence_BiCvp_Users"},
    )

    t_job_Sequence_BiCvp_Users = DataStageOperator(
        task_id="Sequence_BiCvp_Users",
        project=PROJECT_NAME,
        job_name="Sequence_BiCvp_Users",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
        verbose_log=True,
    )

    t_end_Sequence_BiCvp_Users = PythonOperator(
        task_id="log_end_Sequence_BiCvp_Users",
        python_callable=log_end,
        op_kwargs={"job_name": "Sequence_BiCvp_Users", "task_key": "Sequence_BiCvp_Users", "upstream_task_id": "Sequence_BiCvp_Users"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_BiCvp_Users >> t_teams_start >> t_job_Sequence_BiCvp_Users >> t_end_Sequence_BiCvp_Users

    end_tasks = [t_end_Sequence_BiCvp_Users]

    [t_end_Sequence_BiCvp_Users] >> t_publish_dataset

    [t_end_Sequence_BiCvp_Users] >> t_teams_end

    [t_end_Sequence_BiCvp_Users] >> t_teams_error
