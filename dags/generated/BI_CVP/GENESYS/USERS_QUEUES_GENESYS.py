...
# Pipeline: USERS_QUEUES_GENESYS | Projeto: BI_CVP | Dominio: GENESYS
# =========================
DAG_ID        = "USERS_QUEUES_GENESYS"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "GENESYS"
PIPELINE_NAME = "USERS_QUEUES_GENESYS"
...
with DAG(
    ...
    description="Pipeline USERS_QUEUES_GENESYS - BI_CVP / GENESYS",
    start_date=pendulum.datetime(2026, 6, 18, tz=LOCAL_TZ),
    schedule="0 4 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'GENESYS', 'USERS', 'QUEUES', 'CVP'],
) as dag:
    ...
    t_start_Sequence_BiCvp_UsersQueues = PythonOperator(
        task_id="log_start_Sequence_BiCvp_UsersQueues",
        python_callable=log_start,
        op_kwargs={"job_name": "Sequence_BiCvp_UsersQueues", "task_key": "Sequence_BiCvp_UsersQueues"},
    )

    t_job_Sequence_BiCvp_UsersQueues = DataStageOperator(
        task_id="Sequence_BiCvp_UsersQueues",
        project=PROJECT_NAME,
        job_name="Sequence_BiCvp_UsersQueues",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
        verbose_log=True,
    )

    t_end_Sequence_BiCvp_UsersQueues = PythonOperator(
        task_id="log_end_Sequence_BiCvp_UsersQueues",
        python_callable=log_end,
        op_kwargs={"job_name": "Sequence_BiCvp_UsersQueues", "task_key": "Sequence_BiCvp_UsersQueues", "upstream_task_id": "Sequence_BiCvp_UsersQueues"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_BiCvp_UsersQueues >> t_teams_start >> t_job_Sequence_BiCvp_UsersQueues >> t_end_Sequence_BiCvp_UsersQueues

    end_tasks = [t_end_Sequence_BiCvp_UsersQueues]

    [t_end_Sequence_BiCvp_UsersQueues] >> t_publish_dataset

    [t_end_Sequence_BiCvp_UsersQueues] >> t_teams_end

    [t_end_Sequence_BiCvp_UsersQueues] >> t_teams_error
