...
# Pipeline: DATASTAGE_SEQ_EXEC_PD_CONTRATOS | Projeto: BI_CVP | Dominio: BASE_UNICA_CLIENTES
# =========================
DAG_ID        = "DATASTAGE_SEQ_EXEC_PD_CONTRATOS"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "BASE_UNICA_CLIENTES"
PIPELINE_NAME = "DATASTAGE_SEQ_EXEC_PD_CONTRATOS"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SEQ_EXEC_PD_CONTRATOS - BI_CVP / BASE_UNICA_CLIENTES",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="0 9 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'BASE_UNICA_CLIENTES', 'CONTRATOS', 'CLIENTES', 'DIARIO'],
) as dag:
    ...
    t_start_Seq_Exec_PD_CONTRATOS = PythonOperator(
        task_id='log_start_Seq_Exec_PD_CONTRATOS',
        python_callable=log_start,
        op_kwargs={"job_name": 'Seq_Exec_PD_CONTRATOS', "task_key": 'Seq_Exec_PD_CONTRATOS'},
    )

    t_job_Seq_Exec_PD_CONTRATOS = DataStageOperator(
        task_id='Seq_Exec_PD_CONTRATOS',
        project=PROJECT_NAME,
        job_name='Seq_Exec_PD_CONTRATOS',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_Seq_Exec_PD_CONTRATOS = PythonOperator(
        task_id='log_end_Seq_Exec_PD_CONTRATOS',
        python_callable=log_end,
        op_kwargs={"job_name": 'Seq_Exec_PD_CONTRATOS', "task_key": 'Seq_Exec_PD_CONTRATOS', "upstream_task_id": 'Seq_Exec_PD_CONTRATOS'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Seq_Exec_PD_CONTRATOS >> t_job_Seq_Exec_PD_CONTRATOS >> t_end_Seq_Exec_PD_CONTRATOS

    end_tasks = [t_end_Seq_Exec_PD_CONTRATOS]

    [t_end_Seq_Exec_PD_CONTRATOS] >> t_publish_dataset

    [t_end_Seq_Exec_PD_CONTRATOS] >> t_teams_error
