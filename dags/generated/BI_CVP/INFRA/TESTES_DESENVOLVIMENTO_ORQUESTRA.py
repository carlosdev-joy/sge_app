...
from utils.conditions import eval_condition
...
# Pipeline: TESTES_DESENVOLVIMENTO_ORQUESTRA | Projeto: BI_CVP | Dominio: INFRA
# =========================
DAG_ID        = "TESTES_DESENVOLVIMENTO_ORQUESTRA"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "INFRA"
PIPELINE_NAME = "TESTES_DESENVOLVIMENTO_ORQUESTRA"
...
DATASET_URI   = "orq://pipeline/TESTES_DESENVOLVIMENTO_ORQUESTRA"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['DECISAO_1', 'Obtem_data_pipeline', 'RESULTADO_NAO', 'RESULTADO_SIM']
...
with DAG(
    ...
    description="Pipeline TESTES_DESENVOLVIMENTO_ORQUESTRA - BI_CVP / INFRA",
    start_date=pendulum.datetime(2026, 1, 1, tz=LOCAL_TZ),
    schedule="0 6 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'INFRA', 'TESTE'],
) as dag:
    ...
    def _decide_DECISAO_1(**context):
        cond = {'tipo': 'valor_sql', 'source_job': 'Obtem_data_pipeline', 'comparacao': 'data', 'operador': '>', 'valor': 'HOJE', 'on_error': 'falhar', 'ramo_verdadeiro': ['RESULTADO_SIM'], 'ramo_falso': ['RESULTADO_NAO']}
        _exec_id = context.get("ts_nodash")
        _ti = context.get("ti")
        resultado, _valor = eval_condition(cond, MSSQL_CONN_ID, execution_id=_exec_id, pipeline_name=PIPELINE_NAME, ti=_ti)
        _ramo = "verdadeiro" if resultado else "falso"
        _job = 'DECISAO_1'
        print("[DECISAO " + _job + "] valor=" + str(_valor) + " resultado=" + str(resultado) + " -> ramo " + _ramo)
        context["ti"].xcom_push(key="decisao", value={"valor": str(_valor), "resultado": bool(resultado), "ramo": _ramo})
        return ['RESULTADO_SIM'] if resultado else ['RESULTADO_NAO']

    t_dec_DECISAO_1 = BranchPythonOperator(
        task_id='DECISAO_1',
        python_callable=_decide_DECISAO_1,
    )

    def _sql_Obtem_data_pipeline(**context):
        _job = 'Obtem_data_pipeline'
        _sql = "select cast(last_execution as date) as execucao from etl_pipeline where pipeline_name = 'DATASTAGE_ASSISTENCIA_MKT_CLOUD'\n\n\n"
        _conn_id = 'SQL14_DMDB41'
        _database = 'DMDB41'
        _valor = _resolve_e_roda_sql(_sql, _conn_id, _database, context, on_error='falhar')
        print("[SQL NODE " + _job + "] valor publicado=" + repr(_valor))
        return _valor
    t_sql_Obtem_data_pipeline = PythonOperator(
        task_id='Obtem_data_pipeline',
        python_callable=_sql_Obtem_data_pipeline,
    )

    def _notify_RESULTADO_NAO(**context):
        _job = 'RESULTADO_NAO'
        _grupo_id = 1
        _template_id = 2
        _mensagem = 'TESTE NÃO'
        _up_jobs = ['Obtem_data_pipeline']
        _exec_id = context.get("ts_nodash")
        _resolve_e_envia_notificacao(_job, _grupo_id, _template_id, _mensagem, _up_jobs, _exec_id, context)
    t_notif_RESULTADO_NAO = PythonOperator(
        task_id='RESULTADO_NAO',
        python_callable=_notify_RESULTADO_NAO,
        trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS,
    )

    def _notify_RESULTADO_SIM(**context):
        _job = 'RESULTADO_SIM'
        _grupo_id = 1
        _template_id = 1
        _mensagem = 'TESTE DATA FUNCIONANDO'
        _up_jobs = ['Obtem_data_pipeline']
        _exec_id = context.get("ts_nodash")
        _resolve_e_envia_notificacao(_job, _grupo_id, _template_id, _mensagem, _up_jobs, _exec_id, context)
    t_notif_RESULTADO_SIM = PythonOperator(
        task_id='RESULTADO_SIM',
        python_callable=_notify_RESULTADO_SIM,
        trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS,
    )

    t_sql_Obtem_data_pipeline >> t_dec_DECISAO_1

    t_check_agenda >> t_sql_Obtem_data_pipeline

    t_dec_DECISAO_1 >> t_notif_RESULTADO_NAO

    t_dec_DECISAO_1 >> t_notif_RESULTADO_SIM

    end_tasks = []

    [] >> t_publish_dataset

    t_notif_RESULTADO_NAO >> t_publish_dataset

    t_notif_RESULTADO_SIM >> t_publish_dataset

    t_sql_Obtem_data_pipeline >> t_publish_dataset
