...
from utils.job_operators import ShellOperator
...
# Pipeline: SHELL_LIMPA_FS_DATASTAGE | Projeto: BI_CVP | Dominio: INFRA
# =========================
DAG_ID        = "SHELL_LIMPA_FS_DATASTAGE"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "INFRA"
PIPELINE_NAME = "SHELL_LIMPA_FS_DATASTAGE"
...
HORARIOS_ESPECIFICOS = ['23:00']
DIAS_HORARIOS_MES = None
DATASET_URI   = "orq://pipeline/SHELL_LIMPA_FS_DATASTAGE"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['EXEC_ALL_ZIPS.sh', 'Informa_Infra']
FLOW_JOBS     = ['EXEC_ALL_ZIPS.sh']
...
with DAG(
    ...
    description="Pipeline SHELL_LIMPA_FS_DATASTAGE - BI_CVP / INFRA",
    start_date=pendulum.datetime(2026, 1, 1, tz=LOCAL_TZ),
    schedule="0 23 * * 0,2,4",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'INFRA', 'FS_DATASTAGE'],
) as dag:
    ...
    t_start_EXEC_ALL_ZIPS_sh = PythonOperator(
        task_id='log_start_EXEC_ALL_ZIPS.sh',
        python_callable=log_start,
        op_kwargs={"job_name": 'EXEC_ALL_ZIPS.sh', "task_key": 'EXEC_ALL_ZIPS.sh'},
    )

    t_job_EXEC_ALL_ZIPS_sh = ShellOperator(
        task_id='EXEC_ALL_ZIPS.sh',
        ssh_conn_id="ssh_lnxprd021",
        command='cd /Projetos/BI_CVP/Scripts/Scripts_THG/Purge && ./EXEC_ALL_ZIPS.sh',
        cmd_timeout=None,
        do_xcom_push=True,
    )

    t_end_EXEC_ALL_ZIPS_sh = PythonOperator(
        task_id='log_end_EXEC_ALL_ZIPS.sh',
        python_callable=log_end,
        op_kwargs={"job_name": 'EXEC_ALL_ZIPS.sh', "task_key": 'EXEC_ALL_ZIPS.sh', "upstream_task_id": 'EXEC_ALL_ZIPS.sh'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    def _notify_Informa_Infra(**context):
        _job = 'Informa_Infra'
        _grupo_id = 2
        _template_id = 3
        _mensagem = ''
        _up_jobs = ['EXEC_ALL_ZIPS.sh']
    ...
    t_notif_Informa_Infra = PythonOperator(
        task_id='Informa_Infra',
        python_callable=_notify_Informa_Infra,
    )

    t_check_agenda >> t_start_EXEC_ALL_ZIPS_sh >> t_job_EXEC_ALL_ZIPS_sh >> t_end_EXEC_ALL_ZIPS_sh

    t_end_EXEC_ALL_ZIPS_sh >> t_notif_Informa_Infra

    end_tasks = [t_end_EXEC_ALL_ZIPS_sh]

    [t_end_EXEC_ALL_ZIPS_sh] >> t_publish_dataset

    t_notif_Informa_Infra >> t_publish_dataset
