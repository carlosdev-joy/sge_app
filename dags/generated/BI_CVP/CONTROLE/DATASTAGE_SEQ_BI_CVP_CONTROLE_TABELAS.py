...
# Pipeline: DATASTAGE_SEQ_BI_CVP_CONTROLE_TABELAS | Projeto: BI_CVP | Dominio: CONTROLE
# =========================
DAG_ID        = "DATASTAGE_SEQ_BI_CVP_CONTROLE_TABELAS"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "CONTROLE"
PIPELINE_NAME = "DATASTAGE_SEQ_BI_CVP_CONTROLE_TABELAS"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SEQ_BI_CVP_CONTROLE_TABELAS - BI_CVP / CONTROLE",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="30 9 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'CONTROLE'],
) as dag:
    ...
    t_start_SeqBiCvp_ControleTabelas = PythonOperator(
        task_id='log_start_SeqBiCvp_ControleTabelas',
        python_callable=log_start,
        op_kwargs={"job_name": 'SeqBiCvp_ControleTabelas', "task_key": 'SeqBiCvp_ControleTabelas'},
    )

    t_job_SeqBiCvp_ControleTabelas = DataStageOperator(
        task_id='SeqBiCvp_ControleTabelas',
        project=PROJECT_NAME,
        job_name='SeqBiCvp_ControleTabelas',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqBiCvp_ControleTabelas = PythonOperator(
        task_id='log_end_SeqBiCvp_ControleTabelas',
        python_callable=log_end,
        op_kwargs={"job_name": 'SeqBiCvp_ControleTabelas', "task_key": 'SeqBiCvp_ControleTabelas', "upstream_task_id": 'SeqBiCvp_ControleTabelas'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqBiCvp_ControleTabelas >> t_job_SeqBiCvp_ControleTabelas >> t_end_SeqBiCvp_ControleTabelas

    end_tasks = [t_end_SeqBiCvp_ControleTabelas]

    [t_end_SeqBiCvp_ControleTabelas] >> t_publish_dataset

    [t_end_SeqBiCvp_ControleTabelas] >> t_teams_error
