...
# Pipeline: DATASTAGE_SEGMENTACAO_CLIENTES_DIARIO | Projeto: BI_CVP | Dominio: BUCC
# =========================
DAG_ID        = "DATASTAGE_SEGMENTACAO_CLIENTES_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "BUCC"
PIPELINE_NAME = "DATASTAGE_SEGMENTACAO_CLIENTES_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_SEGMENTACAO_CLIENTES_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['BiCvp_ArqSegmentacaoCliente']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SEGMENTACAO_CLIENTES_DIARIO - BI_CVP / BUCC",
    start_date=pendulum.datetime(2026, 6, 19, tz=LOCAL_TZ),
    schedule="0 13 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'BUCC', 'DATASTAGE', 'DIARIO'],
) as dag:
    ...
    t_start_BiCvp_ArqSegmentacaoCliente = PythonOperator(
        task_id="log_start_BiCvp_ArqSegmentacaoCliente",
        python_callable=log_start,
        op_kwargs={"job_name": "BiCvp_ArqSegmentacaoCliente", "task_key": "BiCvp_ArqSegmentacaoCliente"},
    )

    t_job_BiCvp_ArqSegmentacaoCliente = DataStageOperator(
        task_id="BiCvp_ArqSegmentacaoCliente",
        project=PROJECT_NAME,
        job_name="BiCvp_ArqSegmentacaoCliente",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_BiCvp_ArqSegmentacaoCliente = PythonOperator(
        task_id="log_end_BiCvp_ArqSegmentacaoCliente",
        python_callable=log_end,
        op_kwargs={"job_name": "BiCvp_ArqSegmentacaoCliente", "task_key": "BiCvp_ArqSegmentacaoCliente", "upstream_task_id": "BiCvp_ArqSegmentacaoCliente"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_BiCvp_ArqSegmentacaoCliente >> t_job_BiCvp_ArqSegmentacaoCliente >> t_end_BiCvp_ArqSegmentacaoCliente

    end_tasks = [t_end_BiCvp_ArqSegmentacaoCliente]

    [t_end_BiCvp_ArqSegmentacaoCliente] >> t_publish_dataset

    [t_end_BiCvp_ArqSegmentacaoCliente] >> t_teams_error
