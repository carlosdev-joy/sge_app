...
# Pipeline: DATASTAGE_BUCC_BENEFICIARIOS_CONTRATOS_DIARIO | Projeto: BI_CVP | Dominio: BUCC
# =========================
DAG_ID        = "DATASTAGE_BUCC_BENEFICIARIOS_CONTRATOS_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "BUCC"
PIPELINE_NAME = "DATASTAGE_BUCC_BENEFICIARIOS_CONTRATOS_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_BUCC_BENEFICIARIOS_CONTRATOS_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SEQ_BENEFICIARIOS_CONTRATOS_CVP']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_BUCC_BENEFICIARIOS_CONTRATOS_DIARIO - BI_CVP / BUCC",
    start_date=pendulum.datetime(2026, 6, 16, tz=LOCAL_TZ),
    schedule="0 4 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'BUCC', 'DATASTAGE', 'BENEFICIARIOS', 'CONTRATO', 'DIARIO'],
) as dag:
    ...
    t_start_SEQ_BENEFICIARIOS_CONTRATOS_CVP = PythonOperator(
        task_id="log_start_SEQ_BENEFICIARIOS_CONTRATOS_CVP",
        python_callable=log_start,
        op_kwargs={"job_name": "SEQ_BENEFICIARIOS_CONTRATOS_CVP", "task_key": "SEQ_BENEFICIARIOS_CONTRATOS_CVP"},
    )

    t_job_SEQ_BENEFICIARIOS_CONTRATOS_CVP = DataStageOperator(
        task_id="SEQ_BENEFICIARIOS_CONTRATOS_CVP",
        project=PROJECT_NAME,
        job_name="SEQ_BENEFICIARIOS_CONTRATOS_CVP",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_BENEFICIARIOS_CONTRATOS_CVP = PythonOperator(
        task_id="log_end_SEQ_BENEFICIARIOS_CONTRATOS_CVP",
        python_callable=log_end,
        op_kwargs={"job_name": "SEQ_BENEFICIARIOS_CONTRATOS_CVP", "task_key": "SEQ_BENEFICIARIOS_CONTRATOS_CVP", "upstream_task_id": "SEQ_BENEFICIARIOS_CONTRATOS_CVP"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SEQ_BENEFICIARIOS_CONTRATOS_CVP >> t_teams_start >> t_job_SEQ_BENEFICIARIOS_CONTRATOS_CVP >> t_end_SEQ_BENEFICIARIOS_CONTRATOS_CVP

    end_tasks = [t_end_SEQ_BENEFICIARIOS_CONTRATOS_CVP]

    [t_end_SEQ_BENEFICIARIOS_CONTRATOS_CVP] >> t_publish_dataset

    [t_end_SEQ_BENEFICIARIOS_CONTRATOS_CVP] >> t_teams_end

    [t_end_SEQ_BENEFICIARIOS_CONTRATOS_CVP] >> t_teams_error
