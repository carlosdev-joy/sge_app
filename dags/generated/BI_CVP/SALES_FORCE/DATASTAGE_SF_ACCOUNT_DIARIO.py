...
# Pipeline: DATASTAGE_SF_ACCOUNT_DIARIO | Projeto: BI_CVP | Dominio: SALES_FORCE
# =========================
DAG_ID        = "DATASTAGE_SF_ACCOUNT_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SALES_FORCE"
PIPELINE_NAME = "DATASTAGE_SF_ACCOUNT_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_SF_ACCOUNT_DIARIO"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SF_ACCOUNT_DIARIO - BI_CVP / SALES_FORCE",
    start_date=pendulum.datetime(2026, 6, 30, tz=LOCAL_TZ),
    schedule="55 2 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'SALES_FORCE', 'DATASTAGE', 'DIARIO', 'ACCOUNT'],
) as dag:
    ...
    t_start_SEQ_SF_ACCOUNT = PythonOperator(
        task_id='log_start_SEQ_SF_ACCOUNT',
        python_callable=log_start,
        op_kwargs={"job_name": 'SEQ_SF_ACCOUNT', "task_key": 'SEQ_SF_ACCOUNT'},
    )

    t_job_SEQ_SF_ACCOUNT = DataStageOperator(
        task_id='SEQ_SF_ACCOUNT',
        project=PROJECT_NAME,
        job_name='SEQ_SF_ACCOUNT',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_SF_ACCOUNT = PythonOperator(
        task_id='log_end_SEQ_SF_ACCOUNT',
        python_callable=log_end,
        op_kwargs={"job_name": 'SEQ_SF_ACCOUNT', "task_key": 'SEQ_SF_ACCOUNT', "upstream_task_id": 'SEQ_SF_ACCOUNT'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SEQ_SF_ACCOUNT >> t_job_SEQ_SF_ACCOUNT >> t_end_SEQ_SF_ACCOUNT

    end_tasks = [t_end_SEQ_SF_ACCOUNT]

    [t_end_SEQ_SF_ACCOUNT] >> t_publish_dataset

    [t_end_SEQ_SF_ACCOUNT] >> t_teams_error
