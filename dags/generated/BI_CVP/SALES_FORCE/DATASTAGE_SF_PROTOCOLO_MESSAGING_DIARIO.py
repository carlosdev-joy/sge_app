...
# Pipeline: DATASTAGE_SF_PROTOCOLO_MESSAGING_DIARIO | Projeto: BI_CVP | Dominio: SALES_FORCE
# =========================
DAG_ID        = "DATASTAGE_SF_PROTOCOLO_MESSAGING_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SALES_FORCE"
PIPELINE_NAME = "DATASTAGE_SF_PROTOCOLO_MESSAGING_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_SF_PROTOCOLO_MESSAGING_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SEQ_SF_PROTOCOLO_MESSAGING']
FLOW_JOBS     = ['SEQ_SF_PROTOCOLO_MESSAGING']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SF_PROTOCOLO_MESSAGING_DIARIO - BI_CVP / SALES_FORCE",
    start_date=pendulum.datetime(2026, 8, 11, tz=LOCAL_TZ),
    schedule="0 15 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'SALES_FORCE', 'DATASTAGE', 'DIARIO', 'PROTOCOLO', 'MESSAGING'],
) as dag:
    ...
    t_start_SEQ_SF_PROTOCOLO_MESSAGING = _LogStart(
        task_id='log_start_SEQ_SF_PROTOCOLO_MESSAGING',
        python_callable=log_start,
        op_kwargs={"job_name": 'SEQ_SF_PROTOCOLO_MESSAGING', "task_key": 'SEQ_SF_PROTOCOLO_MESSAGING'},
    )

    t_job_SEQ_SF_PROTOCOLO_MESSAGING = DataStageOperator(
        task_id='SEQ_SF_PROTOCOLO_MESSAGING',
        project=PROJECT_NAME,
        job_name='SEQ_SF_PROTOCOLO_MESSAGING',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_SF_PROTOCOLO_MESSAGING = PythonOperator(
        task_id='log_end_SEQ_SF_PROTOCOLO_MESSAGING',
        python_callable=log_end,
        op_kwargs={"job_name": 'SEQ_SF_PROTOCOLO_MESSAGING', "task_key": 'SEQ_SF_PROTOCOLO_MESSAGING', "upstream_task_id": 'SEQ_SF_PROTOCOLO_MESSAGING'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SEQ_SF_PROTOCOLO_MESSAGING >> t_job_SEQ_SF_PROTOCOLO_MESSAGING >> t_end_SEQ_SF_PROTOCOLO_MESSAGING

    end_tasks = [t_end_SEQ_SF_PROTOCOLO_MESSAGING]

    [t_end_SEQ_SF_PROTOCOLO_MESSAGING] >> t_publish_dataset

    [t_end_SEQ_SF_PROTOCOLO_MESSAGING] >> t_teams_error

    [t_end_SEQ_SF_PROTOCOLO_MESSAGING] >> t_reg_falha
