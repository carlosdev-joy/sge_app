...
# Pipeline: DATASTAGE_SF_INCIDENT_INCIDHIST_DIARIO | Projeto: BI_CVP | Dominio: SALES_FORCE
# =========================
DAG_ID        = "DATASTAGE_SF_INCIDENT_INCIDHIST_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SALES_FORCE"
PIPELINE_NAME = "DATASTAGE_SF_INCIDENT_INCIDHIST_DIARIO"
...
HORARIOS_ESPECIFICOS = ['13:00', '17:30']
DIAS_HORARIOS_MES = None
DATASET_URI   = "orq://pipeline/DATASTAGE_SF_INCIDENT_INCIDHIST_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SEQ_SF_INCIDENT', 'SEQ_SF_INCIDENT_HISTORY']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SF_INCIDENT_INCIDHIST_DIARIO - BI_CVP / SALES_FORCE",
    start_date=pendulum.datetime(2026, 6, 18, tz=LOCAL_TZ),
    schedule="0,30 13,17 * * 1,2,3,4,5",
    catchup=False,
    max_active_runs=1,
    ...
    t_start_SEQ_SF_INCIDENT = PythonOperator(
        task_id="log_start_SEQ_SF_INCIDENT",
        python_callable=log_start,
        op_kwargs={"job_name": "SEQ_SF_INCIDENT", "task_key": "SEQ_SF_INCIDENT"},
    )

    t_job_SEQ_SF_INCIDENT = DataStageOperator(
        task_id="SEQ_SF_INCIDENT",
        project=PROJECT_NAME,
        job_name="SEQ_SF_INCIDENT",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_SF_INCIDENT = PythonOperator(
        task_id="log_end_SEQ_SF_INCIDENT",
        python_callable=log_end,
        op_kwargs={"job_name": "SEQ_SF_INCIDENT", "task_key": "SEQ_SF_INCIDENT", "upstream_task_id": "SEQ_SF_INCIDENT"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_start_SEQ_SF_INCIDENT_HISTORY = PythonOperator(
        task_id="log_start_SEQ_SF_INCIDENT_HISTORY",
        python_callable=log_start,
        op_kwargs={"job_name": "SEQ_SF_INCIDENT_HISTORY", "task_key": "SEQ_SF_INCIDENT_HISTORY"},
    )

    t_job_SEQ_SF_INCIDENT_HISTORY = DataStageOperator(
        task_id="SEQ_SF_INCIDENT_HISTORY",
        project=PROJECT_NAME,
        job_name="SEQ_SF_INCIDENT_HISTORY",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_SF_INCIDENT_HISTORY = PythonOperator(
        task_id="log_end_SEQ_SF_INCIDENT_HISTORY",
        python_callable=log_end,
        op_kwargs={"job_name": "SEQ_SF_INCIDENT_HISTORY", "task_key": "SEQ_SF_INCIDENT_HISTORY", "upstream_task_id": "SEQ_SF_INCIDENT_HISTORY"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SEQ_SF_INCIDENT >> t_job_SEQ_SF_INCIDENT >> t_end_SEQ_SF_INCIDENT

    t_check_agenda >> t_start_SEQ_SF_INCIDENT_HISTORY >> t_job_SEQ_SF_INCIDENT_HISTORY >> t_end_SEQ_SF_INCIDENT_HISTORY

    end_tasks = [t_end_SEQ_SF_INCIDENT, t_end_SEQ_SF_INCIDENT_HISTORY]

    [t_end_SEQ_SF_INCIDENT, t_end_SEQ_SF_INCIDENT_HISTORY] >> t_publish_dataset

    [t_end_SEQ_SF_INCIDENT, t_end_SEQ_SF_INCIDENT_HISTORY] >> t_teams_error
