...
# Pipeline: DATASTAGE_SF_BI_DIARIO | Projeto: BI_CVP | Dominio: SALES_FORCE
# =========================
DAG_ID        = "DATASTAGE_SF_BI_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SALES_FORCE"
PIPELINE_NAME = "DATASTAGE_SF_BI_DIARIO"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SF_BI_DIARIO - BI_CVP / SALES_FORCE",
    start_date=pendulum.datetime(2026, 6, 19, tz=LOCAL_TZ),
    schedule="0 4 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'SALES_FORCE', 'DATASTAGE', 'BI', 'DIARIO'],
) as dag:
    ...
    t_start_SeqBiCvp_SalesForce = PythonOperator(
        task_id="log_start_SeqBiCvp_SalesForce",
        python_callable=log_start,
        op_kwargs={"job_name": "SeqBiCvp_SalesForce", "task_key": "SeqBiCvp_SalesForce"},
    )

    t_job_SeqBiCvp_SalesForce = DataStageOperator(
        task_id="SeqBiCvp_SalesForce",
        project=PROJECT_NAME,
        job_name="SeqBiCvp_SalesForce",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqBiCvp_SalesForce = PythonOperator(
        task_id="log_end_SeqBiCvp_SalesForce",
        python_callable=log_end,
        op_kwargs={"job_name": "SeqBiCvp_SalesForce", "task_key": "SeqBiCvp_SalesForce", "upstream_task_id": "SeqBiCvp_SalesForce"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqBiCvp_SalesForce >> t_job_SeqBiCvp_SalesForce >> t_end_SeqBiCvp_SalesForce

    end_tasks = [t_end_SeqBiCvp_SalesForce]

    [t_end_SeqBiCvp_SalesForce] >> t_publish_dataset

    [t_end_SeqBiCvp_SalesForce] >> t_teams_error
