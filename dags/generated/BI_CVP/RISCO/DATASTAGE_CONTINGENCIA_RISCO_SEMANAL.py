...
# Pipeline: DATASTAGE_CONTINGENCIA_RISCO_SEMANAL | Projeto: BI_CVP | Dominio: RISCO
# =========================
DAG_ID        = "DATASTAGE_CONTINGENCIA_RISCO_SEMANAL"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "RISCO"
PIPELINE_NAME = "DATASTAGE_CONTINGENCIA_RISCO_SEMANAL"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_CONTINGENCIA_RISCO_SEMANAL"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['BiCvp_ContingenciaRisco_auto']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_CONTINGENCIA_RISCO_SEMANAL - BI_CVP / RISCO",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="0 7 * * 0",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'RISCO', 'DATASTAGE', 'SEMANAL'],
) as dag:
    ...
    t_start_BiCvp_ContingenciaRisco_auto = PythonOperator(
        task_id="log_start_BiCvp_ContingenciaRisco_auto",
        python_callable=log_start,
        op_kwargs={"job_name": "BiCvp_ContingenciaRisco_auto", "task_key": "BiCvp_ContingenciaRisco_auto"},
    )

    t_job_BiCvp_ContingenciaRisco_auto = DataStageOperator(
        task_id="BiCvp_ContingenciaRisco_auto",
        project=PROJECT_NAME,
        job_name="BiCvp_ContingenciaRisco_auto",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_BiCvp_ContingenciaRisco_auto = PythonOperator(
        task_id="log_end_BiCvp_ContingenciaRisco_auto",
        python_callable=log_end,
        op_kwargs={"job_name": "BiCvp_ContingenciaRisco_auto", "task_key": "BiCvp_ContingenciaRisco_auto", "upstream_task_id": "BiCvp_ContingenciaRisco_auto"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_BiCvp_ContingenciaRisco_auto >> t_job_BiCvp_ContingenciaRisco_auto >> t_end_BiCvp_ContingenciaRisco_auto

    end_tasks = [t_end_BiCvp_ContingenciaRisco_auto]

    [t_end_BiCvp_ContingenciaRisco_auto] >> t_publish_dataset

    [t_end_BiCvp_ContingenciaRisco_auto] >> t_teams_error
