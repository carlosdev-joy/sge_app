...
# Pipeline: DATASTAGE_MONITOR_DADOS_CADASTRAIS_DIARIO | Projeto: BI_CVP | Dominio: RISCO
# =========================
DAG_ID        = "DATASTAGE_MONITOR_DADOS_CADASTRAIS_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "RISCO"
PIPELINE_NAME = "DATASTAGE_MONITOR_DADOS_CADASTRAIS_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_MONITOR_DADOS_CADASTRAIS_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SEQ_MONITOR_DADOS_CADASTRAIS']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_MONITOR_DADOS_CADASTRAIS_DIARIO - BI_CVP / RISCO",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="0 5 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'RISCO', 'DATASTAGE', 'MONITOR', 'DIARIO', 'DADOS_CADASTRAIS'],
) as dag:
    ...
    t_start_SEQ_MONITOR_DADOS_CADASTRAIS = PythonOperator(
        task_id="log_start_SEQ_MONITOR_DADOS_CADASTRAIS",
        python_callable=log_start,
        op_kwargs={"job_name": "SEQ_MONITOR_DADOS_CADASTRAIS", "task_key": "SEQ_MONITOR_DADOS_CADASTRAIS"},
    )

    t_job_SEQ_MONITOR_DADOS_CADASTRAIS = DataStageOperator(
        task_id="SEQ_MONITOR_DADOS_CADASTRAIS",
        project=PROJECT_NAME,
        job_name="SEQ_MONITOR_DADOS_CADASTRAIS",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_MONITOR_DADOS_CADASTRAIS = PythonOperator(
        task_id="log_end_SEQ_MONITOR_DADOS_CADASTRAIS",
        python_callable=log_end,
        op_kwargs={"job_name": "SEQ_MONITOR_DADOS_CADASTRAIS", "task_key": "SEQ_MONITOR_DADOS_CADASTRAIS", "upstream_task_id": "SEQ_MONITOR_DADOS_CADASTRAIS"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SEQ_MONITOR_DADOS_CADASTRAIS >> t_job_SEQ_MONITOR_DADOS_CADASTRAIS >> t_end_SEQ_MONITOR_DADOS_CADASTRAIS

    end_tasks = [t_end_SEQ_MONITOR_DADOS_CADASTRAIS]

    [t_end_SEQ_MONITOR_DADOS_CADASTRAIS] >> t_publish_dataset

    [t_end_SEQ_MONITOR_DADOS_CADASTRAIS] >> t_teams_error
