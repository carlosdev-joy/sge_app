...
# Pipeline: DATASTAGE_CONTINGENCIA_RISCO_REINCIDENCIA_DIARIO | Projeto: BI_CVP | Dominio: RISCO
# =========================
DAG_ID        = "DATASTAGE_CONTINGENCIA_RISCO_REINCIDENCIA_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "RISCO"
PIPELINE_NAME = "DATASTAGE_CONTINGENCIA_RISCO_REINCIDENCIA_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_CONTINGENCIA_RISCO_REINCIDENCIA_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['BiCvp_ContingenciaRisco_reincidencia']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_CONTINGENCIA_RISCO_REINCIDENCIA_DIARIO - BI_CVP / RISCO",
    start_date=pendulum.datetime(2026, 6, 20, tz=LOCAL_TZ),
    schedule="0 3 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'RISCO', 'DATASTAGE', 'DIARIO'],
) as dag:
    ...
    t_start_BiCvp_ContingenciaRisco_reincidencia = PythonOperator(
        task_id="log_start_BiCvp_ContingenciaRisco_reincidencia",
        python_callable=log_start,
        op_kwargs={"job_name": "BiCvp_ContingenciaRisco_reincidencia", "task_key": "BiCvp_ContingenciaRisco_reincidencia"},
    )

    t_job_BiCvp_ContingenciaRisco_reincidencia = DataStageOperator(
        task_id="BiCvp_ContingenciaRisco_reincidencia",
        project=PROJECT_NAME,
        job_name="BiCvp_ContingenciaRisco_reincidencia",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_BiCvp_ContingenciaRisco_reincidencia = PythonOperator(
        task_id="log_end_BiCvp_ContingenciaRisco_reincidencia",
        python_callable=log_end,
        op_kwargs={"job_name": "BiCvp_ContingenciaRisco_reincidencia", "task_key": "BiCvp_ContingenciaRisco_reincidencia", "upstream_task_id": "BiCvp_ContingenciaRisco_reincidencia"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_BiCvp_ContingenciaRisco_reincidencia >> t_job_BiCvp_ContingenciaRisco_reincidencia >> t_end_BiCvp_ContingenciaRisco_reincidencia

    end_tasks = [t_end_BiCvp_ContingenciaRisco_reincidencia]

    [t_end_BiCvp_ContingenciaRisco_reincidencia] >> t_publish_dataset

    [t_end_BiCvp_ContingenciaRisco_reincidencia] >> t_teams_error
