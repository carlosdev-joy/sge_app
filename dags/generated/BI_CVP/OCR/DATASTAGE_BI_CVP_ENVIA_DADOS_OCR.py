...
# Pipeline: DATASTAGE_BI_CVP_ENVIA_DADOS_OCR | Projeto: BI_CVP | Dominio: OCR
# =========================
DAG_ID        = "DATASTAGE_BI_CVP_ENVIA_DADOS_OCR"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "OCR"
PIPELINE_NAME = "DATASTAGE_BI_CVP_ENVIA_DADOS_OCR"
...
HORARIOS_ESPECIFICOS = ['08:00', '18:00']
DIAS_HORARIOS_MES = None
DATASET_URI   = "orq://pipeline/DATASTAGE_BI_CVP_ENVIA_DADOS_OCR"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['Sequence_BiCvp_ENVIA_DADOS_OCR']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_BI_CVP_ENVIA_DADOS_OCR - BI_CVP / OCR",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="0 8,18 * * 1,2,3,4,5",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'OCR'],
) as dag:
    ...
    t_start_Sequence_BiCvp_ENVIA_DADOS_OCR = PythonOperator(
        task_id='log_start_Sequence_BiCvp_ENVIA_DADOS_OCR',
        python_callable=log_start,
        op_kwargs={"job_name": 'Sequence_BiCvp_ENVIA_DADOS_OCR', "task_key": 'Sequence_BiCvp_ENVIA_DADOS_OCR'},
    )

    t_job_Sequence_BiCvp_ENVIA_DADOS_OCR = DataStageOperator(
        task_id='Sequence_BiCvp_ENVIA_DADOS_OCR',
        project=PROJECT_NAME,
        job_name='Sequence_BiCvp_ENVIA_DADOS_OCR',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_Sequence_BiCvp_ENVIA_DADOS_OCR = PythonOperator(
        task_id='log_end_Sequence_BiCvp_ENVIA_DADOS_OCR',
        python_callable=log_end,
        op_kwargs={"job_name": 'Sequence_BiCvp_ENVIA_DADOS_OCR', "task_key": 'Sequence_BiCvp_ENVIA_DADOS_OCR', "upstream_task_id": 'Sequence_BiCvp_ENVIA_DADOS_OCR'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_BiCvp_ENVIA_DADOS_OCR >> t_job_Sequence_BiCvp_ENVIA_DADOS_OCR >> t_end_Sequence_BiCvp_ENVIA_DADOS_OCR

    end_tasks = [t_end_Sequence_BiCvp_ENVIA_DADOS_OCR]

    [t_end_Sequence_BiCvp_ENVIA_DADOS_OCR] >> t_publish_dataset

    [t_end_Sequence_BiCvp_ENVIA_DADOS_OCR] >> t_teams_error
