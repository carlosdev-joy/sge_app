...
# Pipeline: DATASTAGE_ASSISTENCIA_MKT_CLOUD | Projeto: BI_CVP | Dominio: MKT_CLOUD
# =========================
DAG_ID        = "DATASTAGE_ASSISTENCIA_MKT_CLOUD"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "MKT_CLOUD"
PIPELINE_NAME = "DATASTAGE_ASSISTENCIA_MKT_CLOUD"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_ASSISTENCIA_MKT_CLOUD"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['BiCvp_Assistencia_00_ext_mkt_cloud', 'Informacao', 'BiCvp_Assistencia_01_ins_mkt_cloud']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_ASSISTENCIA_MKT_CLOUD - BI_CVP / MKT_CLOUD",
    start_date=pendulum.datetime(2026, 1, 1, tz=LOCAL_TZ),
    schedule="30 7 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'MKT_CLOUD', 'ASSISTENCIA', 'DIARIO', 'API', 'BUCC'],
) as dag:
    ...
    t_start_BiCvp_Assistencia_00_ext_mkt_cloud = PythonOperator(
        task_id='log_start_BiCvp_Assistencia_00_ext_mkt_cloud',
        python_callable=log_start,
        op_kwargs={"job_name": 'BiCvp_Assistencia_00_ext_mkt_cloud', "task_key": 'BiCvp_Assistencia_00_ext_mkt_cloud'},
    )

    t_job_BiCvp_Assistencia_00_ext_mkt_cloud = DataStageOperator(
        task_id='BiCvp_Assistencia_00_ext_mkt_cloud',
        project=PROJECT_NAME,
        job_name='BiCvp_Assistencia_00_ext_mkt_cloud',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_BiCvp_Assistencia_00_ext_mkt_cloud = PythonOperator(
        task_id='log_end_BiCvp_Assistencia_00_ext_mkt_cloud',
        python_callable=log_end,
        op_kwargs={"job_name": 'BiCvp_Assistencia_00_ext_mkt_cloud', "task_key": 'BiCvp_Assistencia_00_ext_mkt_cloud', "upstream_task_id": 'BiCvp_Assistencia_00_ext_mkt_cloud'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    def _notify_Informacao(**context):
        _job = 'Informacao'
        _grupo_id = 1
        _template_id = None
        _mensagem = '{pipeline} executado com sucesso. total de >> {linhas} carregadas.\n\nStatus: {status}\nData: {data}'
        _up_jobs = ['BiCvp_Assistencia_00_ext_mkt_cloud']
    ...
    t_notif_Informacao = PythonOperator(
        task_id='Informacao',
        python_callable=_notify_Informacao,
    )

    t_start_BiCvp_Assistencia_01_ins_mkt_cloud = PythonOperator(
        task_id='log_start_BiCvp_Assistencia_01_ins_mkt_cloud',
        python_callable=log_start,
        op_kwargs={"job_name": 'BiCvp_Assistencia_01_ins_mkt_cloud', "task_key": 'BiCvp_Assistencia_01_ins_mkt_cloud'},
    )

    t_job_BiCvp_Assistencia_01_ins_mkt_cloud = DataStageOperator(
        task_id='BiCvp_Assistencia_01_ins_mkt_cloud',
        project=PROJECT_NAME,
        job_name='BiCvp_Assistencia_01_ins_mkt_cloud',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_BiCvp_Assistencia_01_ins_mkt_cloud = PythonOperator(
        task_id='log_end_BiCvp_Assistencia_01_ins_mkt_cloud',
        python_callable=log_end,
        op_kwargs={"job_name": 'BiCvp_Assistencia_01_ins_mkt_cloud', "task_key": 'BiCvp_Assistencia_01_ins_mkt_cloud', "upstream_task_id": 'BiCvp_Assistencia_01_ins_mkt_cloud'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_BiCvp_Assistencia_00_ext_mkt_cloud >> t_job_BiCvp_Assistencia_00_ext_mkt_cloud >> t_end_BiCvp_Assistencia_00_ext_mkt_cloud

    t_end_BiCvp_Assistencia_00_ext_mkt_cloud >> t_notif_Informacao

    t_notif_Informacao >> t_start_BiCvp_Assistencia_01_ins_mkt_cloud >> t_job_BiCvp_Assistencia_01_ins_mkt_cloud >> t_end_BiCvp_Assistencia_01_ins_mkt_cloud

    end_tasks = [t_end_BiCvp_Assistencia_00_ext_mkt_cloud, t_end_BiCvp_Assistencia_01_ins_mkt_cloud]

    [t_end_BiCvp_Assistencia_00_ext_mkt_cloud, t_end_BiCvp_Assistencia_01_ins_mkt_cloud] >> t_publish_dataset

    [t_end_BiCvp_Assistencia_00_ext_mkt_cloud, t_end_BiCvp_Assistencia_01_ins_mkt_cloud] >> t_teams_end

    [t_end_BiCvp_Assistencia_00_ext_mkt_cloud, t_end_BiCvp_Assistencia_01_ins_mkt_cloud] >> t_teams_error

    t_notif_Informacao >> t_publish_dataset

    t_notif_Informacao >> t_teams_end

    t_notif_Informacao >> t_teams_error
