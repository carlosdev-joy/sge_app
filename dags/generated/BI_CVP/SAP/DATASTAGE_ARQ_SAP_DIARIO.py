...
# Pipeline: DATASTAGE_ARQ_SAP_DIARIO | Projeto: BI_CVP | Dominio: SAP
# =========================
DAG_ID        = "DATASTAGE_ARQ_SAP_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SAP"
PIPELINE_NAME = "DATASTAGE_ARQ_SAP_DIARIO"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_ARQ_SAP_DIARIO - BI_CVP / SAP",
    start_date=pendulum.datetime(2026, 6, 16, tz=LOCAL_TZ),
    schedule="40 16 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'SAP', 'DATASTAGE', 'ARQH', 'DIARIO'],
) as dag:
    ...
    t_start_SeqCvpBi_ArqSAP = PythonOperator(
        task_id="log_start_SeqCvpBi_ArqSAP",
        python_callable=log_start,
        op_kwargs={"job_name": "SeqCvpBi_ArqSAP", "task_key": "SeqCvpBi_ArqSAP"},
    )

    t_job_SeqCvpBi_ArqSAP = DataStageOperator(
        task_id="SeqCvpBi_ArqSAP",
        project=PROJECT_NAME,
        job_name="SeqCvpBi_ArqSAP",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqCvpBi_ArqSAP = PythonOperator(
        task_id="log_end_SeqCvpBi_ArqSAP",
        python_callable=log_end,
        op_kwargs={"job_name": "SeqCvpBi_ArqSAP", "task_key": "SeqCvpBi_ArqSAP", "upstream_task_id": "SeqCvpBi_ArqSAP"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqCvpBi_ArqSAP >> t_job_SeqCvpBi_ArqSAP >> t_end_SeqCvpBi_ArqSAP

    end_tasks = [t_end_SeqCvpBi_ArqSAP]

    [t_end_SeqCvpBi_ArqSAP] >> t_publish_dataset

    [t_end_SeqCvpBi_ArqSAP] >> t_teams_error
