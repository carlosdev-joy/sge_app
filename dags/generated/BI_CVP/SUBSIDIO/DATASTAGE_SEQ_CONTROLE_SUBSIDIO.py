...
# Pipeline: DATASTAGE_SEQ_CONTROLE_SUBSIDIO | Projeto: BI_CVP | Dominio: SUBSIDIO
# =========================
DAG_ID        = "DATASTAGE_SEQ_CONTROLE_SUBSIDIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SUBSIDIO"
PIPELINE_NAME = "DATASTAGE_SEQ_CONTROLE_SUBSIDIO"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SEQ_CONTROLE_SUBSIDIO - BI_CVP / SUBSIDIO",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="30 9 * * 1,2,3,4,5",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'SUBSIDIO'],
) as dag:
    ...
    t_start_SEQ_CONTROLE_SUBSIDIO = PythonOperator(
        task_id='log_start_SEQ_CONTROLE_SUBSIDIO',
        python_callable=log_start,
        op_kwargs={"job_name": 'SEQ_CONTROLE_SUBSIDIO', "task_key": 'SEQ_CONTROLE_SUBSIDIO'},
    )

    t_job_SEQ_CONTROLE_SUBSIDIO = DataStageOperator(
        task_id='SEQ_CONTROLE_SUBSIDIO',
        project=PROJECT_NAME,
        job_name='SEQ_CONTROLE_SUBSIDIO',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SEQ_CONTROLE_SUBSIDIO = PythonOperator(
        task_id='log_end_SEQ_CONTROLE_SUBSIDIO',
        python_callable=log_end,
        op_kwargs={"job_name": 'SEQ_CONTROLE_SUBSIDIO', "task_key": 'SEQ_CONTROLE_SUBSIDIO', "upstream_task_id": 'SEQ_CONTROLE_SUBSIDIO'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SEQ_CONTROLE_SUBSIDIO >> t_job_SEQ_CONTROLE_SUBSIDIO >> t_end_SEQ_CONTROLE_SUBSIDIO

    end_tasks = [t_end_SEQ_CONTROLE_SUBSIDIO]

    [t_end_SEQ_CONTROLE_SUBSIDIO] >> t_publish_dataset

    [t_end_SEQ_CONTROLE_SUBSIDIO] >> t_teams_error
