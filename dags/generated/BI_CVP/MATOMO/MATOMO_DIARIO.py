...
# Pipeline: MATOMO_DIARIO | Projeto: BI_CVP | Dominio: MATOMO
# =========================
DAG_ID        = "MATOMO_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "MATOMO"
PIPELINE_NAME = "MATOMO_DIARIO"
...
with DAG(
    ...
    description="Pipeline MATOMO_DIARIO - BI_CVP / MATOMO",
    start_date=pendulum.datetime(2026, 6, 18, tz=LOCAL_TZ),
    schedule="0 6 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'MATOMO', 'BICVP', 'DIARIO'],
) as dag:
    ...
    t_start_Sequence_BiCvp_Matomo = PythonOperator(
        task_id="log_start_Sequence_BiCvp_Matomo",
        python_callable=log_start,
        op_kwargs={"job_name": "Sequence_BiCvp_Matomo", "task_key": "Sequence_BiCvp_Matomo"},
    )

    t_job_Sequence_BiCvp_Matomo = DataStageOperator(
        task_id="Sequence_BiCvp_Matomo",
        project=PROJECT_NAME,
        job_name="Sequence_BiCvp_Matomo",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_Sequence_BiCvp_Matomo = PythonOperator(
        task_id="log_end_Sequence_BiCvp_Matomo",
        python_callable=log_end,
        op_kwargs={"job_name": "Sequence_BiCvp_Matomo", "task_key": "Sequence_BiCvp_Matomo", "upstream_task_id": "Sequence_BiCvp_Matomo"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_BiCvp_Matomo >> t_teams_start >> t_job_Sequence_BiCvp_Matomo >> t_end_Sequence_BiCvp_Matomo

    end_tasks = [t_end_Sequence_BiCvp_Matomo]

    [t_end_Sequence_BiCvp_Matomo] >> t_publish_dataset

    [t_end_Sequence_BiCvp_Matomo] >> t_teams_end

    [t_end_Sequence_BiCvp_Matomo] >> t_teams_error
