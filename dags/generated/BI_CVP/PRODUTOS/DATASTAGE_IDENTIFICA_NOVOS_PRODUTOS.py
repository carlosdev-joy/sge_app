...
# Pipeline: DATASTAGE_IDENTIFICA_NOVOS_PRODUTOS | Projeto: BI_CVP | Dominio: PRODUTOS
# =========================
DAG_ID        = "DATASTAGE_IDENTIFICA_NOVOS_PRODUTOS"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "PRODUTOS"
PIPELINE_NAME = "DATASTAGE_IDENTIFICA_NOVOS_PRODUTOS"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_IDENTIFICA_NOVOS_PRODUTOS"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['CvpBi_SEQ_IdentificaNovosProdutos']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_IDENTIFICA_NOVOS_PRODUTOS - BI_CVP / PRODUTOS",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="0 12 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'PRODUTOS'],
) as dag:
    ...
    t_start_CvpBi_SEQ_IdentificaNovosProdutos = PythonOperator(
        task_id='log_start_CvpBi_SEQ_IdentificaNovosProdutos',
        python_callable=log_start,
        op_kwargs={"job_name": 'CvpBi_SEQ_IdentificaNovosProdutos', "task_key": 'CvpBi_SEQ_IdentificaNovosProdutos'},
    )

    t_job_CvpBi_SEQ_IdentificaNovosProdutos = DataStageOperator(
        task_id='CvpBi_SEQ_IdentificaNovosProdutos',
        project=PROJECT_NAME,
        job_name='CvpBi_SEQ_IdentificaNovosProdutos',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_CvpBi_SEQ_IdentificaNovosProdutos = PythonOperator(
        task_id='log_end_CvpBi_SEQ_IdentificaNovosProdutos',
        python_callable=log_end,
        op_kwargs={"job_name": 'CvpBi_SEQ_IdentificaNovosProdutos', "task_key": 'CvpBi_SEQ_IdentificaNovosProdutos', "upstream_task_id": 'CvpBi_SEQ_IdentificaNovosProdutos'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_CvpBi_SEQ_IdentificaNovosProdutos >> t_job_CvpBi_SEQ_IdentificaNovosProdutos >> t_end_CvpBi_SEQ_IdentificaNovosProdutos

    end_tasks = [t_end_CvpBi_SEQ_IdentificaNovosProdutos]

    [t_end_CvpBi_SEQ_IdentificaNovosProdutos] >> t_publish_dataset

    [t_end_CvpBi_SEQ_IdentificaNovosProdutos] >> t_teams_error
