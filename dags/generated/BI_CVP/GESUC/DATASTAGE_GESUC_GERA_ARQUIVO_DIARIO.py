...
# Pipeline: DATASTAGE_GESUC_GERA_ARQUIVO_DIARIO | Projeto: BI_CVP | Dominio: GESUC
# =========================
DAG_ID        = "DATASTAGE_GESUC_GERA_ARQUIVO_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "GESUC"
PIPELINE_NAME = "DATASTAGE_GESUC_GERA_ARQUIVO_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_GESUC_GERA_ARQUIVO_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 3, "retry_delay": timedelta(seconds=300)}
JOBS          = ['Sequence_CvpBi_GeraArquivo_GESUC']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_GESUC_GERA_ARQUIVO_DIARIO - BI_CVP / GESUC",
    start_date=pendulum.datetime(2026, 6, 16, tz=LOCAL_TZ),
    schedule="30 7 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'GESUC', 'DATASTAGE', 'VIEW', 'ARQUIVO', 'DIARIO'],
) as dag:
    ...
    t_start_Sequence_CvpBi_GeraArquivo_GESUC = PythonOperator(
        task_id="log_start_Sequence_CvpBi_GeraArquivo_GESUC",
        python_callable=log_start,
        op_kwargs={"job_name": "Sequence_CvpBi_GeraArquivo_GESUC", "task_key": "Sequence_CvpBi_GeraArquivo_GESUC"},
    )

    t_job_Sequence_CvpBi_GeraArquivo_GESUC = DataStageOperator(
        task_id="Sequence_CvpBi_GeraArquivo_GESUC",
        project=PROJECT_NAME,
        job_name="Sequence_CvpBi_GeraArquivo_GESUC",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_Sequence_CvpBi_GeraArquivo_GESUC = PythonOperator(
        task_id="log_end_Sequence_CvpBi_GeraArquivo_GESUC",
        python_callable=log_end,
        op_kwargs={"job_name": "Sequence_CvpBi_GeraArquivo_GESUC", "task_key": "Sequence_CvpBi_GeraArquivo_GESUC", "upstream_task_id": "Sequence_CvpBi_GeraArquivo_GESUC"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_Sequence_CvpBi_GeraArquivo_GESUC >> t_teams_start >> t_job_Sequence_CvpBi_GeraArquivo_GESUC >> t_end_Sequence_CvpBi_GeraArquivo_GESUC

    end_tasks = [t_end_Sequence_CvpBi_GeraArquivo_GESUC]

    [t_end_Sequence_CvpBi_GeraArquivo_GESUC] >> t_publish_dataset

    [t_end_Sequence_CvpBi_GeraArquivo_GESUC] >> t_teams_end

    [t_end_Sequence_CvpBi_GeraArquivo_GESUC] >> t_teams_error
