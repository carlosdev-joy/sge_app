...
# Pipeline: DATASTAGE_SEQUICSEGURADORAPRINCIPALDNE | Projeto: BI_CVP | Dominio: SEGURADO
# =========================
DAG_ID        = "DATASTAGE_SEQUICSEGURADORAPRINCIPALDNE"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "SEGURADO"
PIPELINE_NAME = "DATASTAGE_SEQUICSEGURADORAPRINCIPALDNE"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_SEQUICSEGURADORAPRINCIPALDNE"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SeqUICSeguradoraPrincipalDNE']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SEQUICSEGURADORAPRINCIPALDNE - BI_CVP / SEGURADO",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="30 16 * * 5",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'SEGURADO'],
) as dag:
    ...
    t_start_SeqUICSeguradoraPrincipalDNE = PythonOperator(
        task_id='log_start_SeqUICSeguradoraPrincipalDNE',
        python_callable=log_start,
        op_kwargs={"job_name": 'SeqUICSeguradoraPrincipalDNE', "task_key": 'SeqUICSeguradoraPrincipalDNE'},
    )

    t_job_SeqUICSeguradoraPrincipalDNE = DataStageOperator(
        task_id='SeqUICSeguradoraPrincipalDNE',
        project=PROJECT_NAME,
        job_name='SeqUICSeguradoraPrincipalDNE',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqUICSeguradoraPrincipalDNE = PythonOperator(
        task_id='log_end_SeqUICSeguradoraPrincipalDNE',
        python_callable=log_end,
        op_kwargs={"job_name": 'SeqUICSeguradoraPrincipalDNE', "task_key": 'SeqUICSeguradoraPrincipalDNE', "upstream_task_id": 'SeqUICSeguradoraPrincipalDNE'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqUICSeguradoraPrincipalDNE >> t_job_SeqUICSeguradoraPrincipalDNE >> t_end_SeqUICSeguradoraPrincipalDNE

    end_tasks = [t_end_SeqUICSeguradoraPrincipalDNE]

    [t_end_SeqUICSeguradoraPrincipalDNE] >> t_publish_dataset

    [t_end_SeqUICSeguradoraPrincipalDNE] >> t_teams_error
