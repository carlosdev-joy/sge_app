...
# Pipeline: DATASTAGE_BICVP_SEQ_ASSESSORIAJURIDICA | Projeto: BI_CVP | Dominio: ASSESSORIA_JURIDICA
# =========================
DAG_ID        = "DATASTAGE_BICVP_SEQ_ASSESSORIAJURIDICA"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_CVP"
DOMAIN        = "ASSESSORIA_JURIDICA"
PIPELINE_NAME = "DATASTAGE_BICVP_SEQ_ASSESSORIAJURIDICA"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_BICVP_SEQ_ASSESSORIAJURIDICA"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['BiCvp_SEQ_AssessoriaJuridica']
FLOW_JOBS     = ['BiCvp_SEQ_AssessoriaJuridica']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_BICVP_SEQ_ASSESSORIAJURIDICA - BI_CVP / ASSESSORIA_JURIDICA",
    start_date=pendulum.datetime(2026, 6, 21, tz=LOCAL_TZ),
    schedule="45 6 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_CVP', 'ASSESSORIA_JURIDICA', 'ASSESSORIA'],
) as dag:
    ...
    t_start_BiCvp_SEQ_AssessoriaJuridica = PythonOperator(
        task_id='log_start_BiCvp_SEQ_AssessoriaJuridica',
        python_callable=log_start,
        op_kwargs={"job_name": 'BiCvp_SEQ_AssessoriaJuridica', "task_key": 'BiCvp_SEQ_AssessoriaJuridica'},
    )

    t_job_BiCvp_SEQ_AssessoriaJuridica = DataStageOperator(
        task_id='BiCvp_SEQ_AssessoriaJuridica',
        project=PROJECT_NAME,
        job_name='BiCvp_SEQ_AssessoriaJuridica',
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_BiCvp_SEQ_AssessoriaJuridica = PythonOperator(
        task_id='log_end_BiCvp_SEQ_AssessoriaJuridica',
        python_callable=log_end,
        op_kwargs={"job_name": 'BiCvp_SEQ_AssessoriaJuridica', "task_key": 'BiCvp_SEQ_AssessoriaJuridica', "upstream_task_id": 'BiCvp_SEQ_AssessoriaJuridica'},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_BiCvp_SEQ_AssessoriaJuridica >> t_job_BiCvp_SEQ_AssessoriaJuridica >> t_end_BiCvp_SEQ_AssessoriaJuridica

    end_tasks = [t_end_BiCvp_SEQ_AssessoriaJuridica]

    [t_end_BiCvp_SEQ_AssessoriaJuridica] >> t_publish_dataset

    [t_end_BiCvp_SEQ_AssessoriaJuridica] >> t_teams_error
