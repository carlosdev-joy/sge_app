...
# Pipeline: DATASTAGE_PROPOSTAS_DECLINADAS_ERRO | Projeto: BI_PRESTAMISTA | Dominio: CARGA
# =========================
DAG_ID        = "DATASTAGE_PROPOSTAS_DECLINADAS_ERRO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_PRESTAMISTA"
DOMAIN        = "CARGA"
PIPELINE_NAME = "DATASTAGE_PROPOSTAS_DECLINADAS_ERRO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_PROPOSTAS_DECLINADAS_ERRO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 2, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SeqSsdPrs_PropDeclinadasErro_auto']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_PROPOSTAS_DECLINADAS_ERRO - BI_PRESTAMISTA / CARGA",
    start_date=pendulum.datetime(2026, 6, 17, tz=LOCAL_TZ),
    schedule="0 8 * * 2,3,4,5,6",
    catchup=False,
    max_active_runs=1,
    tags=['BI_PRESTAMISTA', 'CARGA', 'DATASTAGE', 'PROPOSTAS', 'DECLINADAS', 'ERRO', 'AUTO', 'PRESTAMISTA'],
) as dag:
    ...
    t_start_SeqSsdPrs_PropDeclinadasErro_auto = PythonOperator(
        task_id="log_start_SeqSsdPrs_PropDeclinadasErro_auto",
        python_callable=log_start,
        op_kwargs={"job_name": "SeqSsdPrs_PropDeclinadasErro_auto", "task_key": "SeqSsdPrs_PropDeclinadasErro_auto"},
    )

    t_job_SeqSsdPrs_PropDeclinadasErro_auto = DataStageOperator(
        task_id="SeqSsdPrs_PropDeclinadasErro_auto",
        project=PROJECT_NAME,
        job_name="SeqSsdPrs_PropDeclinadasErro_auto",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqSsdPrs_PropDeclinadasErro_auto = PythonOperator(
        task_id="log_end_SeqSsdPrs_PropDeclinadasErro_auto",
        python_callable=log_end,
        op_kwargs={"job_name": "SeqSsdPrs_PropDeclinadasErro_auto", "task_key": "SeqSsdPrs_PropDeclinadasErro_auto", "upstream_task_id": "SeqSsdPrs_PropDeclinadasErro_auto"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqSsdPrs_PropDeclinadasErro_auto >> t_teams_start >> t_job_SeqSsdPrs_PropDeclinadasErro_auto >> t_end_SeqSsdPrs_PropDeclinadasErro_auto

    end_tasks = [t_end_SeqSsdPrs_PropDeclinadasErro_auto]

    [t_end_SeqSsdPrs_PropDeclinadasErro_auto] >> t_publish_dataset

    [t_end_SeqSsdPrs_PropDeclinadasErro_auto] >> t_teams_end

    [t_end_SeqSsdPrs_PropDeclinadasErro_auto] >> t_teams_error
