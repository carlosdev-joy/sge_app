...
# Pipeline: DATASTAGE_CARGA_DIARIA | Projeto: BI_PRESTAMISTA | Dominio: CARGA
# =========================
DAG_ID        = "DATASTAGE_CARGA_DIARIA"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_PRESTAMISTA"
DOMAIN        = "CARGA"
PIPELINE_NAME = "DATASTAGE_CARGA_DIARIA"
...
with DAG(
    ...
    description="Pipeline DATASTAGE_CARGA_DIARIA - BI_PRESTAMISTA / CARGA",
    start_date=pendulum.datetime(2026, 6, 16, tz=LOCAL_TZ),
    schedule="20 5 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_PRESTAMISTA', 'CARGA', 'DIARIA', 'PRESTAMISTA', 'DATASTAGE'],
) as dag:
    ...
    t_start_SeqSsdPrs_CargaDiaria = PythonOperator(
        task_id="log_start_SeqSsdPrs_CargaDiaria",
        python_callable=log_start,
        op_kwargs={"job_name": "SeqSsdPrs_CargaDiaria", "task_key": "SeqSsdPrs_CargaDiaria"},
    )

    t_job_SeqSsdPrs_CargaDiaria = DataStageOperator(
        task_id="SeqSsdPrs_CargaDiaria",
        project=PROJECT_NAME,
        job_name="SeqSsdPrs_CargaDiaria",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqSsdPrs_CargaDiaria = PythonOperator(
        task_id="log_end_SeqSsdPrs_CargaDiaria",
        python_callable=log_end,
        op_kwargs={"job_name": "SeqSsdPrs_CargaDiaria", "task_key": "SeqSsdPrs_CargaDiaria", "upstream_task_id": "SeqSsdPrs_CargaDiaria"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqSsdPrs_CargaDiaria >> t_teams_start >> t_job_SeqSsdPrs_CargaDiaria >> t_end_SeqSsdPrs_CargaDiaria

    end_tasks = [t_end_SeqSsdPrs_CargaDiaria]

    [t_end_SeqSsdPrs_CargaDiaria] >> t_publish_dataset

    [t_end_SeqSsdPrs_CargaDiaria] >> t_teams_end

    [t_end_SeqSsdPrs_CargaDiaria] >> t_teams_error
