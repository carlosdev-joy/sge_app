...
# Pipeline: DATASTAGE_SINI_INDEFERIDOS_SEG_DIARIO | Projeto: BI_PRESTAMISTA | Dominio: CARGA
# =========================
DAG_ID        = "DATASTAGE_SINI_INDEFERIDOS_SEG_DIARIO"
SSH_CONN_ID   = "ssh_lnxprd021"
MSSQL_CONN_ID = "SQL14_DMDB41"
PROJECT_NAME  = "BI_PRESTAMISTA"
DOMAIN        = "CARGA"
PIPELINE_NAME = "DATASTAGE_SINI_INDEFERIDOS_SEG_DIARIO"
...
DATASET_URI   = "orq://pipeline/DATASTAGE_SINI_INDEFERIDOS_SEG_DIARIO"
default_args  = {"owner": "airflow", "depends_on_past": False, "retries": 1, "retry_delay": timedelta(seconds=300)}
JOBS          = ['SeqSsdPrs_SiniIndeferidosSeg_auto']
...
with DAG(
    ...
    description="Pipeline DATASTAGE_SINI_INDEFERIDOS_SEG_DIARIO - BI_PRESTAMISTA / CARGA",
    start_date=pendulum.datetime(2026, 6, 17, tz=LOCAL_TZ),
    schedule="0 9 * * *",
    catchup=False,
    max_active_runs=1,
    tags=['BI_PRESTAMISTA', 'CARGA', 'DATASTAGE', 'SINI', 'INDEFERIDOS', 'DIARIO', 'SEG', 'PRESTAMISTA'],
) as dag:
    ...
    t_start_SeqSsdPrs_SiniIndeferidosSeg_auto = PythonOperator(
        task_id="log_start_SeqSsdPrs_SiniIndeferidosSeg_auto",
        python_callable=log_start,
        op_kwargs={"job_name": "SeqSsdPrs_SiniIndeferidosSeg_auto", "task_key": "SeqSsdPrs_SiniIndeferidosSeg_auto"},
    )

    t_job_SeqSsdPrs_SiniIndeferidosSeg_auto = DataStageOperator(
        task_id="SeqSsdPrs_SiniIndeferidosSeg_auto",
        project=PROJECT_NAME,
        job_name="SeqSsdPrs_SiniIndeferidosSeg_auto",
        ssh_conn_id=SSH_CONN_ID,
        queue_name=DS_QUEUE,
    )

    t_end_SeqSsdPrs_SiniIndeferidosSeg_auto = PythonOperator(
        task_id="log_end_SeqSsdPrs_SiniIndeferidosSeg_auto",
        python_callable=log_end,
        op_kwargs={"job_name": "SeqSsdPrs_SiniIndeferidosSeg_auto", "task_key": "SeqSsdPrs_SiniIndeferidosSeg_auto", "upstream_task_id": "SeqSsdPrs_SiniIndeferidosSeg_auto"},
        trigger_rule=TriggerRule.ALL_DONE,
    )

    t_check_agenda >> t_start_SeqSsdPrs_SiniIndeferidosSeg_auto >> t_teams_start >> t_job_SeqSsdPrs_SiniIndeferidosSeg_auto >> t_end_SeqSsdPrs_SiniIndeferidosSeg_auto

    end_tasks = [t_end_SeqSsdPrs_SiniIndeferidosSeg_auto]

    [t_end_SeqSsdPrs_SiniIndeferidosSeg_auto] >> t_publish_dataset

    [t_end_SeqSsdPrs_SiniIndeferidosSeg_auto] >> t_teams_end

    [t_end_SeqSsdPrs_SiniIndeferidosSeg_auto] >> t_teams_error
