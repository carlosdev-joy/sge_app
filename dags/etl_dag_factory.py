...
def _generate_dag_source(pipeline, jobs):
    ...
    helpers_lines = [
        ...
        '            "SELECT job_name, COALESCE(duration_seconds,0) FROM dbo.etl_job_execution "',
        ...
        '        subtitle=f"O pipeline {PIPELINE_NAME} foi interrompido por falha em um ou mais jobs.",',
        "        facts=[",
        '            _fact("Pipeline",      PIPELINE_NAME),',
        '            _fact("Domínio",       DOMAIN),',
        '            _fact("Projeto",       PROJECT_NAME),',
        '            _fact("Execution ID",  execution_id),',
        '            _fact("Job(s) com falha", failed_names),',
        '            _fact("Horário",       _now_str()),',
        "        ],",
        "        status='FAILED',",
        "    )",
        "",
