...
def _generate_dag_source(pipeline, jobs):
    ...
    import_lines += [
        ...
        "",
        # ── F5 da spec-malha-execucao: o ODATE vem da CORRIDA da malha (§7).
        # Import guardado pela MESMA razão do de cima — e aqui ela é ainda mais
        # dura: sem `utils/malha_corrida.py` no servidor, um import solto
        # derrubaria o carregamento de TODA DAG regenerada, e a data de
        # referência é o eixo de tudo que o motor grava. Com `_corrida = None`,
        # os degraus 0 a 3 do §7 somem e a data volta a ser, byte a byte, a de
        # antes desta fase.
    ...
    helpers_lines = [
        ...
        *([
        ...
        "",
        # ⚠️ A CHAMADA abaixo — o alias `_corrida` mais o nome `odate` mais o
        # parêntese — é a marca sondada pelo §12.2: é por ela que a API sabe,
        # pipeline a pipeline, se a DAG publicada já foi regerada com esta fase.
        # Trocar o alias ou o nome da função sem trocar `MARCA_CORRIDA` em
        # `api/services/espera.py` faz a sonda acusar "ausente" em quem está em
        # dia — o teste de paridade das duas marcas quebra de propósito.
        #
        # Este aviso mora AQUI, na factory, e NÃO no docstring gerado: §12.2
        # exige que a marca seja sintática e "não case em comentário nem em
        # código morto". Escrevê-la no docstring emitido punha uma segunda
        # ocorrência dentro do arquivo publicado — e uma DAG que perdesse a
        # chamada mas guardasse o texto passaria na sonda dizendo OK.
