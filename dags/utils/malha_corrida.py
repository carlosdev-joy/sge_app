...
def corrida_ativa(conn) -> bool:
    ...
    try:
        ...
        print(f"{LOG} interruptor {CHAVE_ATIVA} indisponivel ({e}) — ciclo DESLIGADA")
    _CACHE_ATIVA["ativa"] = valor
    if valor:
        print(f"{LOG} ciclo de malha LIGADA")
    return valor
...
def corridas_abertas(conn) -> list:
    ...
    try:
        ...
        print(f"{LOG} ciclos abertos indisponiveis ({e}) — nada o fechar neste ciclo")
        return []
...
def corrida(conn, corrida_id):
    ...
    try:
        ...
        print(f"{LOG} ciclo #{corrida_id} indisponivel ({e}) — seguindo sem")
        return None
...
def corridas_das_linhas(conn, pipelines, data_ref) -> list:
    ...
    try:
        ...
        print(f"{LOG} ciclos das linhas de {len(alvos)} pipeline(s) em "
              f"{data_ref} indisponiveis ({e}) — nenhum ciclo tocada")
        return []
...
def corrida_aberta_do_pipeline(conn, pipeline: str) -> dict:
    ...
    **`ambiguo` nunca é resolvido por escolha** (Decisão 34): duas corridas
    ...
    try:
        ...
        print(f"{LOG} ciclos abertos de {pipeline} indisponiveis ({e}) — sem ODATE")
        return {"corridas": [], "odate": None, "ambiguo": False}
    odates = {c["data_referencia"] for c in corridas}
    if len(odates) > 1:
        print(f"{LOG} {pipeline} e membro de {len(corridas)} ciclos abertos com "
...
def _corrida_do_conf(conn, conf_id, pipeline: str):
    ...
    try:
        ...
        print(f"{LOG} ciclo #{alvo} do conf indisponivel ({e}) — tratada como AUSENTE")
        return None
    if row is None:
        ...
        print(f"{LOG} conf aponta o ciclo #{alvo}, que nao esta aberta ou nao e "
...
def odate(conn, pipeline: str, run_id=None, conf_id=None, herdada=None) -> dict:
    ...
    if herdada is not None:
        ...
        if aberta["ambiguo"]:
            print(f"{LOG} {pipeline} tem ciclos abertos com ODATEs diferentes, "
            ...
            print(f"{LOG} {pipeline}: data herdada {herdada} difere do ODATE "
                  f"{aberta['odate']} do ciclo aberto — a heranca prevalece e a "
    ...
    if aberta["odate"] is not None:
        ...
        if unica is None:
            print(f"{LOG} {pipeline} e membro de {len(aberta['corridas'])} ciclos "
...
def membros(conn, corrida_id) -> list:
    ...
    try:
        ...
        print(f"{LOG} membros do ciclo #{corrida_id} indisponiveis ({e})")
        return []
    ...
    **Violação de `ux_malha_exec_aberta` não é erro** (Decisão 14): significa que
    ...
    for tentativa in range(1, _TENTATIVAS_ABERTURA + 1):
        try:
            ...
            print(f"{LOG} abertura de {malha} nao devolveu a linha — sem ciclo")
            return None
            ...
            if _violou(e, IX_ABERTA):
                ...
                print(f"{LOG} ciclo de {malha} fechou durante a abertura — "
            ...
            if _violou(e, IX_SEQUENCIA):
                ...
                continue
            if _sem_085(e):
                print(f"{LOG} 085 ausente ({e}) — ciclo de {malha} nao aberta")
                return None
            raise
    print(f"{LOG} abertura de {malha} desistiu apos {_TENTATIVAS_ABERTURA} "
          f"tentativas — sem ciclo neste ciclo")
    return None
...
def vincular_execucao(conn, pipeline: str, data_ref, execution_id,
    ...
    **WRITE-ONCE** (Decisão 9): o `AND malha_execucao_id IS NULL` é o que impede
    ...
    try:
        ...
        if _sem_085(e):
            print(f"{LOG} 085 ausente ({e}) — {pipeline} nao vinculado o ciclo "
                  f"#{corrida_id}")
            return False
        raise
...
def fechar_corrida(conn, corrida_id, desfecho: str, fechada_por: str,
    ...
    try:
        ...
        if _sem_085(e):
            print(f"{LOG} 085 ausente ({e}) — ciclo #{corrida_id} nao fechada")
            return False
        raise
...
def reabrir_corrida(conn, corrida_id, reaberta_por: str, motivo=None) -> bool:
    ...
    try:
        ...
        if _sem_085(e):
            print(f"{LOG} 085 ausente ({e}) — ciclo #{corrida_id} nao reaberta")
            return False
        raise
    if reabriu:
...
def descartar_desfecho(conn, corrida_id) -> int:
    ...
    try:
        ...
        print(f"{LOG} eventos do desfecho do ciclo #{corrida_id} nao "
              f"descartados ({e}) — o ciclo reabriu; o card da proxima "
              f"conclusao pode nao sair")
        return 0
    if n:
        print(f"{LOG} ciclo #{corrida_id}: {n} evento(s) do desfecho anulado "
...
def marcar_visto(conn, corrida_id, o_que: str) -> bool:
    ...
    try:
        ...
        if _sem_085(e):
            print(f"{LOG} 085 ausente ({e}) — marcador {o_que} do ciclo "
...
# worker (medido) e a mesma diferença existe em qualquer servidor com o fuso
# desalinhado.
...
def raizes_da_malha(conn, malha: str) -> list:
    ...
    try:
        ...
        print(f"{LOG} raizes de {malha} indisponiveis ({e}) — porta implicita "
              f"pulada neste ciclo")
        return []
...
_SQL_PARTIDAS = (
    ...
    "FROM dbo.etl_pipeline_execucao e "
    "WHERE {lista} "
    "AND e.data_referencia IN (%s, %s) "
    "AND e.status IN (" + ", ".join("%s" for _ in STATUS_PARTIU) + ") "
    "AND e.substituida_em IS NULL "
    "AND COALESCE(e.inicio, e.criado_em) >= DATEADD(HOUR, -%s, SYSDATETIME()) "
    "AND NOT EXISTS (SELECT 1 FROM dbo.etl_malha_execucao me "
    "WHERE me.malha_name = %s "
...
def sql_partidas(quantos: int) -> str:
    ...
    marcadores = ", ".join("%s" for _ in range(quantos))
    return _SQL_PARTIDAS.format(lista="e.pipeline_name IN (" + marcadores + ")")


def partidas_a_cobrir(conn, malha: str, pipelines, datas, teto_horas: int) -> list:
    ...
    try:
        ...
        print(f"{LOG} partidas de {malha} indisponiveis ({e}) — nenhuma "
              f"abertura neste ciclo")
        return []
...
SQL_FORA_DO_ODATE = (
    ...
    "FROM dbo.etl_pipeline_execucao e "
    "WHERE e.malha_execucao_id = %s AND e.data_referencia <> %s "
    "AND e.substituida_em IS NULL "
...
def estado(conn, corrida: dict, dispensa_sem_linha=None) -> dict:
    ...
    try:
        ...
        print(f"{LOG} estado do ciclo #{corrida_id} indisponivel ({e}) — "
              f"nada o fechar neste ciclo")
        return vazio
    ...
    try:
        ...
        print(f"{LOG} linhas fora do ODATE do ciclo #{corrida_id} "
              f"indisponiveis ({e})")
    return saida
...
SQL_AGUARDANDO_DO_SNAPSHOT = (
    ...
    "ON m.malha_execucao_id = %s AND m.pipeline_name = e.pipeline_name "
    "WHERE e.data_referencia = %s AND e.status = 'AGUARDANDO_DEPENDENCIA' "
    "AND e.substituida_em IS NULL "
    "ORDER BY e.pipeline_name")


def aguardando_do_snapshot(conn, corrida: dict) -> list:
    ...
    try:
        ...
        print(f"{LOG} aguardando do ciclo #{corrida['id']} indisponivel "
...
SQL_RELOGIOS = (
    ...
    "THEN 1 ELSE 0 END, "
    "CASE WHEN me.aberta_em < DATEADD(MINUTE, -%s, SYSDATETIME()) "
    "THEN 1 ELSE 0 END, "
    "CASE WHEN me.aberta_em < DATEADD(MINUTE, -%s, SYSDATETIME()) "
    ...
    "< DATEADD(MINUTE, -%s, SYSDATETIME()) THEN 1 ELSE 0 END "
    "FROM dbo.etl_malha_execucao me WHERE me.id = %s")


def relogios(conn, corrida: dict, carencia_min: int, quiescencia_min: int) -> dict:
    ...
    try:
        ...
        print(f"{LOG} relogios do ciclo #{corrida['id']} indisponiveis ({e}) "
              f"— nenhum desfecho por tempo neste ciclo")
        row = None
    if not row:
...
def corrida_aberta_da_linha(conn, pipeline: str, data_ref):
    ...
    try:
        ...
        if _sem_085(e):
            return None
        print(f"{LOG} ciclo aberto de {pipeline} em {data_ref} indisponivel "
              f"({e}) — fechamento adiado")
...
SQL_CARIMBAR_MOTIVO = (
    "UPDATE dbo.etl_pipeline_execucao "
    "SET motivo = LEFT(ISNULL(motivo + ' | ', '') + %s, 500), "
    "atualizado_em = GETDATE() "
    "WHERE pipeline_name = %s AND data_referencia = %s AND execution_id = %s "
    "AND (motivo IS NULL OR motivo NOT LIKE %s)")


def carimbar_motivo(conn, pipeline: str, data_ref, execution_id,
...
SQL_HEARTBEAT_CRIAR = (
    ...
    "SELECT %s, CONVERT(VARCHAR(19), SYSDATETIME(), 120), %s, %s "
    "WHERE NOT EXISTS (SELECT 1 FROM dbo.etl_app_config WHERE config_key = %s)")
SQL_HEARTBEAT_LER = (
    "SELECT c.config_value, "
    "CASE WHEN TRY_CONVERT(DATETIME2, c.config_value) >= "
    "DATEADD(MINUTE, -%s, SYSDATETIME()) THEN 1 ELSE 0 END "
    "FROM dbo.etl_app_config c WHERE c.config_key = %s")


def marcar_heartbeat(conn, quem: str = "guardia") -> bool:
    ...
    `datetime.now().isoformat()` do worker, que no dev estaria 3h atrás.
    ...
    try:
        ...
        print(f"{LOG} heartbeat da guardia nao gravado ({e}) — a F3 vai tratar "
              f"como guardia ausente")
        return False


def heartbeat_guardia(conn, minutos: int = 15) -> dict:
    ...
    try:
        ...
        print(f"{LOG} heartbeat da guardia indisponivel ({e}) — tratado como "
              f"ausente")
...
SQL_CORRIDA_DA_DATA = (
    ...
    "WHERE malha_name = %s AND data_referencia = %s "
