...
- [x] (E-dev 2026-08-02 F3 — parte F3; redisparo pela guardiã fica p/ F4) **D16** (F3 · EXECUÇÃO no dev) — Trigger que falha (DAG não serializada, worker morto) **devolve a reserva** (`WHERE execution_id IS NULL` — corrida já adotada não é revertida) e a guardiã redispara no ciclo seguinte; nenhuma corrida fica presa em EXECUTANDO sem alerta. [B3]
...
- [x] (E-dev 2026-08-02 F3 — parte F3: push segurou e o 2º pai adotou; guardiã às 08:00 fica p/ F4) **D22** (F3+F4 · EXECUÇÃO no dev) — `nao_iniciar_antes=08:00` com liberação 07:10: o push segura e a guardiã dispara às 08:00 — nem antes, nem nunca. [aceite F3]
