"""ia_provedor.chat_conversa — a conversa multi-rodada que o Maestro usa (F1 da
...
def test_normalizar_junta_papeis_iguais_e_descarta_o_que_vem_antes_do_user():
    msgs = ia_provedor.normalizar_mensagens([
...
def test_normalizar_exige_terminar_em_user(mensagens):
    with pytest.raises(HTTPException) as e:
        ia_provedor.normalizar_mensagens(mensagens)
    assert e.value.status_code == 422


def test_transcrever_uma_mensagem_vai_como_esta_e_historico_rotulado():
    assert ia_provedor.transcrever([{"role": "user", "content": "PERGUNTA"}]) == "PERGUNTA"
    assert ia_provedor.transcrever(HIST) == ("Usuário: mês anterior\n\nAssistente: Quais nomes?\n\n"
...
async def test_chat_de_uma_mensagem_manda_o_corpo_de_sempre(monkeypatch):
    ...
    texto, modelo = await ia_provedor.chat(_cfg(), "INSTRUCAO", "PERGUNTA")
    assert texto == "OK" and modelo == ia_provedor.DEFAULT_MODEL["caixa_gateway"]
...
async def test_chat_conversa_no_gateway_manda_o_transcrito_numa_mensagem(monkeypatch):
    ...
    texto, _ = await ia_provedor.chat_conversa(_cfg(), "SYS", HIST)
    assert texto == "R"
    msgs = _Cliente.ultima.pedido["json"]["messages"]
    assert len(msgs) == 1 and msgs[0]["role"] == "user"
    assert msgs[0]["content"] == "SYS\n\n" + ia_provedor.transcrever(HIST)
...
async def test_chat_conversa_openai_compat_manda_system_mais_a_lista(monkeypatch):
    ...
    texto, modelo = await ia_provedor.chat_conversa(_cfg("openai_compat"), "SYS", HIST)
    assert texto == "R" and modelo == ia_provedor.DEFAULT_MODEL["openai_compat"]
...
async def test_chat_conversa_anthropic_manda_a_lista_alternada(sdk_anthropic):
    texto, modelo = await ia_provedor.chat_conversa(_cfg("anthropic"), "SYS", HIST)
    assert texto == "R" and modelo == ia_provedor.DEFAULT_MODEL["anthropic"]
...
async def test_chat_anthropic_de_uma_mensagem_continua_igual(sdk_anthropic):
    await ia_provedor.chat(_cfg("anthropic"), "SYS", "PERGUNTA")
...
async def test_chat_conversa_valida_antes_de_exigir_a_chave():
    ...
    with pytest.raises(HTTPException) as e:
        await ia_provedor.chat_conversa(_cfg(api_key_enc=""), "SYS", [{"role": "assistant", "content": "x"}])
    assert e.value.status_code == 422
