...
async def test_le_formato_anthropic(monkeypatch):
    ...
    texto, modelo = await ia_provedor.chat(_cfg(), "sistema", "mensagem")
    assert texto == "OK"
...
async def test_le_formato_openai_como_fallback(monkeypatch):
    ...
    texto, _ = await ia_provedor.chat(_cfg(), "sistema", "mensagem")
    assert texto == "OK"
...
async def test_autentica_por_x_api_key(monkeypatch):
    ...
    await ia_provedor.chat(_cfg(), "sistema", "mensagem")
...
async def test_system_viaja_dentro_da_mensagem(monkeypatch):
    ...
    _instala(monkeypatch, _resposta({"content": [{"text": "OK"}]}))
    await ia_provedor.chat(_cfg(), "INSTRUCAO-DO-SISTEMA", "PERGUNTA")
...
async def test_url_recebe_chat_completions(monkeypatch):
    _instala(monkeypatch, _resposta({"content": [{"text": "OK"}]}))
    await ia_provedor.chat(_cfg(), "s", "m")
...
async def test_usa_proxy_quando_configurado(monkeypatch):
    _instala(monkeypatch, _resposta({"content": [{"text": "OK"}]}))
    await ia_provedor.chat(_cfg(usa_proxy=True), "s", "m")
...
def test_proxy_do_ambiente_reporta_variavel(monkeypatch):
    ...
    assert ia_provedor.proxy_do_ambiente() == ""
    monkeypatch.setenv("HTTPS_PROXY", "http://webproxy.empresa:8080")
    assert ia_provedor.proxy_do_ambiente() == "http://webproxy.empresa:8080"
...
async def test_diagnostico_sucesso(monkeypatch):
    _instala(monkeypatch, _resposta({"content": [{"text": "OK"}]}))
    d = await ia_provedor.diagnosticar(_cfg())
    assert d["ok"] is True
    assert d["etapa"] == "ok"
...
async def test_diagnostico_sem_chave_nao_chama_rede(monkeypatch):
    ...
    monkeypatch.setattr(ia_provedor.httpx, "AsyncClient", _explode)
    d = await ia_provedor.diagnosticar(_cfg(api_key_enc=""))
...
async def test_diagnostico_sem_base_url(monkeypatch):
    d = await ia_provedor.diagnosticar(_cfg(base_url=""))
    assert d["ok"] is False
    assert d["etapa"] == "config"
...
async def test_diagnostico_com_proxy_culpa_o_proxy(monkeypatch):
    ...
    d = await ia_provedor.diagnosticar(_cfg(usa_proxy=True))
    assert d["etapa"] == "rede"
...
async def test_erro_antes_de_qualquer_etapa_vira_inesperado(monkeypatch):
    ...
    monkeypatch.setattr(ia_provedor, "_verificar_gateway",
...
def test_gateway_esta_na_lista_de_provedores():
    assert "caixa_gateway" in ia_provedor.PROVIDERS
    assert "caixa_gateway" in ia_provedor.PROVIDERS_COM_BASE_URL
    assert ia_provedor.DEFAULT_MODEL["caixa_gateway"] == "claude-sonnet-4-6"


def test_extrai_texto_reconhece_os_dois_dialetos():
    t, f = ia_provedor.extrai_texto({"content": [{"type": "text", "text": " oi "}]})
    assert (t, "anthropic" in f) == ("oi", True)
    t, f = ia_provedor.extrai_texto({"choices": [{"message": {"content": "oi"}}]})
    assert (t, "openai" in f) == ("oi", True)
    assert ia_provedor.extrai_texto({"nada": 1}) == ("", "")
...
async def test_chave_ilegivel_e_etapa_propria(monkeypatch):
    ...
    monkeypatch.setattr(ia_provedor, "decrypt_password", _falha)
    d = await ia_provedor.diagnosticar(_cfg())
...
async def test_https_recusado_menciona_certificado(monkeypatch):
    ...
    d = await ia_provedor.diagnosticar(_cfg(base_url="https://gw.empresa.intranet/api"))
    assert d["etapa"] == "rede"
...
def test_verificacao_tls_usa_ssl_cert_file(monkeypatch, tmp_path):
    ...
    assert ia_provedor._verificacao_tls() == str(ca)
    monkeypatch.delenv("SSL_CERT_FILE")
    assert ia_provedor._verificacao_tls() is True
