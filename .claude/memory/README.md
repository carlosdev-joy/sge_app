...
- [Spec Maestro — chat de parâmetros DataStage](orquestra-spec-maestro-parametros.md) — 🏁 **SPEC CONCLUÍDA 2026-09-10: F1–F4 MERGEADAS (PRs #382 `2062889`, #383 `bf4a1a9`, #384 `9e0d5d2`, #385 `8758b7e`)**; 🔧 **complemento F5 (Maestro no wizard do pipeline + perguntas conceituais, spec §9) = PR #387 ABERTA, aguarda autorização de merge** — migration 110, chat em Etapas/Fluxos com avatar SVG próprio, Admin (interruptor/catálogo/pedidos, retenção 180d), manual §3.11/§4.9; ⏳ **deploy em produção PENDENTE** (roteiro em `docs/release-notes/maestro.md`: 110 na 6c, `api/`, `dags/` sem restart, `dist/`, `config/` n; depois chave em Caixa Seguro IA e ligar em Admin › Maestro); smoke pós-deploy b/d/h/i/j/k/m só em ambiente real; ⚠️ DEV sem provedor de IA (smoke feito com provedor de mentira no container), F3 Admin catálogo/pedidos, F4 manual/smoke; reusa o provedor `caixa_ia_*`; ⚠️ DEV sem provedor de IA configurado (conversa real só com chave)
