...
- [Spec E-mail: modelos e navegação do anexo](orquestra-spec-email-modelos-navegacao.md) — ✅ APROVADA (modelo "exigido com escape"); ✅ F1 MERGEADA (PR #393 `35bbe2f`, prévia em iframe isolado); ✅ F2 MERGEADA (PR #394 `8089e96`, catálogo + migration 112); 🔧 **F3 em revisão** (seletor de arquivo do anexo, subpasta agora vale); ⚠️ GOTCHA novo: modal aberto sobre o canvas alimentava os atalhos do React Flow — `nokey` no Modal + `haOverlayAberto()`; ⚠️ regra: padronização só vale COM catálogo e a régua do salvar aceita modelo INATIVO; falta a F4 (docs); ⏳ deploy em produção pendente
