...
- [Spec Resultado do SQL no e-mail + 3 correções](orquestra-spec-email-tabela-sql.md) — 🏁 **SPEC CONCLUÍDA 2026-09-11: F1–F6** (PRs #398–#403); `{tabela}` traz o resultado do nó SQL para dentro do aviso; corrigidos o campo de destinatários (só aceitava 1 digitado), o timeout da prévia SQL (15s, e o campo estava escondido na aba de Notificações) e o cabeçalho do modelo no Outlook (VML sem PixelsPerInch/mso-fit-shape-to-text + dark mode); ⏳ **deploy em produção PENDENTE** (113 na 6c, `dags/` com restart do worker e **republicar os pipelines com nó SQL**; roteiro em `docs/release-notes/email-tabela-sql.md`); ⚠️ GOTCHAs: o `text/plain` é gerado por regex e COLAVA as células, comentário HTML citando tag vaza para o texto puro, teste cruzado TS×Python não pega erro que os dois cometem
- [Spec E-mail: modelos e navegação do anexo](orquestra-spec-email-modelos-navegacao.md) — 🏁 **SPEC CONCLUÍDA 2026-09-11: F1–F4 MERGEADAS** (PRs #393 `35bbe2f`, #394 `8089e96`, #395 `b21ae66`, #396); prévia em iframe isolado, catálogo `etl_email_modelo` (migration 112) e seletor de arquivo do anexo; ⏳ **deploy em produção PENDENTE** (112 na 6c + `dags/` com restart do worker; roteiro, conferência e reversão em `docs/release-notes/email-modelos.md`); ⚠️ GOTCHAs: modal aberto sobre o canvas alimentava os atalhos do React Flow (`nokey` no Modal + `haOverlayAberto()`), padronização só vale COM catálogo, a régua do salvar aceita modelo INATIVO
- [Spec Notificação por e-mail](orquestra-spec-notificacao-email.md) — 🏁 **CONCLUÍDA 2026-09-11: F1–F4 MERGEADAS (PRs #388–#391)**; ⏳ **deploy em produção PENDENTE** (111 na 6c, restart do worker, recriar API+worker por env novo, regerar as DAGs, republicar 1× quem tem nó de notificação); roteiro em `docs/release-notes/email.md`
